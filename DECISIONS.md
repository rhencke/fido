...
| ASSUMPTION-GATE-PLACEMENT | OPEN | `rocq c gate/Assumptions.v` costs ≈77 s — 59% of a cold `prove` and 29% of a cold `make check` — while the strictly stronger whole-theory `Fido Audit Assumptions` costs 1.7 s. Options: keep it as is (the default); keep it but move it off the ordinary edit loop to `make audit-fresh` before acceptance review; shrink it to a stated rule. Measured evidence is in Git history at `3b9c103`. | Rob deciding, or a cold-path cost that stops being tolerable. |
