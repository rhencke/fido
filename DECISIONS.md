...
| ASSUMPTION-GATE-PLACEMENT | ACCEPTED | The readable `gate/Assumptions.v` surface list is deleted. The whole-theory `Fido Audit Assumptions` and its adversarial self-tests own the build-time zero-project-axiom claim. The list was a hand-picked subset with no rule deriving it, weaker in coverage than the audit it sat beside, and it cost ≈77 s against the audit's 1.7 s. | Evidence that the whole-theory audit misses a surface the list would have caught. |
