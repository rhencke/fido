...
/-! divergences:
  ...
  * Notation: `Ascii.eqb` / `Nat.eqb` / `Nat.leb` / `Nat.ltb` / `Z.eqb` / `Z.ltb` / `Z.leb` / `String.eqb` are
    `decide` on the propositions, Rocq's `if` on a `bool` is `bif`, `String c s` is `c :: s` (`String c` as a
    function is `(c :: ·)`), `EmptyString` is `[]`, `option_map` is `Option.map`, `Z.of_N n` is `(n : Int)`
    (Syntax.lean: `N` is `Nat`), `Z.opp` is `-`; an arm that uses the whole scrutinee spells it by its pattern
    (`read_nat`, `read_signed_dec`, `read_go_int`: `dval0 (c :: s')` for the `.v`'s `dval0 s`).  `++` is list
    append, which Lean parses left-associated where Rocq parses right; every statement is written in Lean's
    association (`str_app_assoc` bridges where a proof needs the other).
  ...
  * Catch-all `_` match arms are enumerated (README): `expr_is_unary`, `render_names`, `render_group`,
    `decode_string_body`, `decode_decimal_body`, `go_int_lit`.
  * `Example`s are `theorem`s; the five render examples that print a positive go through
    `Decimal.positive_digits` (well-founded) and use `decide +kernel`, about 0.3 s of kernel type checking
    each (`repair_bare_render` 0.30 s, `float_1p5` 0.30 s, `float_1e6` 0.31 s, `float_neg` 0.31 s, `cplx_lit`
    0.32 s): the cost is unfolding `WellFounded.fix`, the same for `15` as for 2^63; Rocq's `reflexivity` was
    structural.  `float_zero` and `cplx_zero` print no positive and stay plain `decide`.
  * Axiom closure (the audit's finding; no statement changes): `propext` and `Quot.sound` enter through
    Decimal.lean's well-founded `positive_digits` (`WellFounded.fix`) into everything that reaches
    `Decimal.integer` or `Decimal.positive` — `signed_Z`, `signed_exp`, `decimal` and the renderer from
    `render_literal` up through `file` / `file_first_line`, the `_ascii` lemmas of those, the faithfulness,
    `str_all_digits_print_Z*`, `integer_nonempty`, `read_signed_dec_render_*`, `read_go_int_*`,
    `go_int_lit_*_literal` and `positive_head_not_minus` lemmas and the seven render examples — through
    Float.lean's `decimal_to_constant` / `constant_zero` into everything that reaches `decode_decimal_body`
    (`decode_decimal`, `decode_render_decimal`), and through `omega` (Rocq: `lia`) alone into `digits_dval`.
    `propext` alone enters through core's `Nat.div_lt_iff_lt_mul` into `hex_escape_ascii`, `string_byte_ascii`,
    `string_body_ascii`, `string_literal_ascii` and the four `_no_nl_cr` lemmas from `hex_escape_no_nl_cr` on,
    and through it, `Nat.div_add_mod` and `UInt8.ofNat_toNat` into `byte_reconstruct`, `hex_escape_exact`,
    `decode_render_byte`, `decode_body_render` and `string_roundtrip`.  Every other declaration is axiom-free:
    `nat_of_ascii_lt_256` (core's `UInt8.toNat_lt`), the `hex_digit` and `digit` lemmas, the identifier /
    binding / type / names / spec-line / group renders and their `_ascii` lemmas, everything about
    `module_file`, the decoder shape lemmas (`hex_digit_decode`, `decode_hex_prefix`, `read_nat_all_digits`,
    `read_signed_dec_all_digits`, `read_signed_dec_sign`, `char_digit_not`, `go_int_lit_all_digits_nonempty`,
    `go_int_lit_neg`) and the eleven byte examples.  Nothing here reaches `Classical.choice`. -/

namespace Fido.Render
open Fido.Syntax (Expr)
open Fido.Syntax.Expr

def nl_c : UInt8 := UInt8.ofNat 10
def tab_c : UInt8 := UInt8.ofNat 9
def nl : Str := [nl_c]
def tab : Str := [tab_c]
...
def cr_c     : UInt8 := UInt8.ofNat 13
def dquote_c : UInt8 := UInt8.ofNat 34   -- the double-quote byte 0x22
def bslash_c : UInt8 := UInt8.ofNat 92

def hex_digit (k : Nat) : UInt8 :=
  bif decide (k < 10) then UInt8.ofNat (48 + k) else UInt8.ofNat (87 + k)

def hex_escape (c : UInt8) : Str :=
  let n := c.toNat
  bslash_c :: byte! 'x' :: hex_digit (n / 16) :: hex_digit (n % 16) :: []

def string_byte (c : UInt8) : Str :=
  let n := c.toNat
  bif decide (n = 34) then bslash_c :: dquote_c :: []
  else bif decide (n = 92) then bslash_c :: bslash_c :: []
  else bif decide (n = 10) then bslash_c :: byte! 'n' :: []
  else bif decide (n = 9)  then bslash_c :: byte! 't' :: []
  else bif decide (n = 13) then bslash_c :: byte! 'r' :: []
...
def signed_Z (z : Int) : Str :=
  bif decide (z < 0) then byte! '-' :: Decimal.integer (-z) else Decimal.integer z

/-- The exponent field carries an explicit sign, so every canonical float spelling is self-delimiting. -/
def signed_exp (e : Int) : Str :=
  bif decide (e < 0) then byte! '-' :: Decimal.integer (-e) else byte! '+' :: Decimal.integer e
...
-- Rocq: `Bool.andb_true_iff`, one direction, by cases (Names.lean).
...
theorem nat_of_ascii_lt_256 : ∀ c : UInt8, c.toNat < 256 := fun c => UInt8.toNat_lt c

theorem hex_digit_ascii : ∀ k, k < 16 → Names.is_ascii_c (hex_digit k) = true := by decide

theorem hex_escape_ascii : ∀ c, Names.str_ascii (hex_escape c) = true := by
  intro c
  have Hb := nat_of_ascii_lt_256 c
  have Hhi : c.toNat / 16 < 16 := (Nat.div_lt_iff_lt_mul (by decide)).2 Hb
  have Hlo : c.toNat % 16 < 16 := Nat.mod_lt _ (by decide)
  show (Names.is_ascii_c bslash_c && (Names.is_ascii_c (byte! 'x')
...
theorem string_byte_ascii : ∀ c, Names.str_ascii (string_byte c) = true := by
  intro c
  unfold string_byte
  dsimp only
  ...
  | true =>
    ...
    rfl
  | false => exact hex_escape_ascii c

theorem string_body_ascii : ∀ s, Names.str_ascii (string_body s) = true := by
  intro s
  induction s with
  | nil => rfl
  | cons c s' IH =>
    show Names.str_ascii (string_byte c ++ string_body s') = true
    rw [str_ascii_app, string_byte_ascii c, IH]
    rfl

theorem string_literal_ascii : ∀ s, Names.str_ascii (string_literal s) = true := by
  intro s
  show (Names.is_ascii_c dquote_c && Names.str_ascii (string_body s ++ dquote_c :: [])) = true
  rw [str_ascii_app, string_body_ascii s]
  decide
...
theorem signed_Z_ascii : ∀ z, Names.str_ascii (signed_Z z) = true := by
  ...
  | true =>
    show (Names.is_ascii_c (byte! '-') && Names.str_ascii (Decimal.integer (-z))) = true
    rw [integer_ascii]
    rfl
...
theorem signed_exp_ascii : ∀ e, Names.str_ascii (signed_exp e) = true := by
  ...
  | true =>
    show (Names.is_ascii_c (byte! '-') && Names.str_ascii (Decimal.integer (-e))) = true
    rw [integer_ascii]
    rfl
  | false =>
    show (Names.is_ascii_c (byte! '+') && Names.str_ascii (Decimal.integer e)) = true
...
theorem render_literal_ascii : ∀ l, Names.str_ascii (render_literal l) = true := by
  intro l
  cases l with
  | IntegerLiteral n => exact integer_ascii (n : Int)
  | FloatLiteral d => exact decimal_ascii _
  | StringLiteral s => exact string_literal_ascii s
...
theorem render_expr_ascii : ∀ e, Names.str_ascii (render_expr e) = true := by
  refine Syntax.Expr_ind' (fun e => Names.str_ascii (render_expr e) = true) ?_ ?_ ?_ ?_
  · intro n; exact Names.render_ordinary_ascii n
  · intro l; exact render_literal_ascii l
  · intro op e IH
    cases op
    ...
    | false =>
      ...
      rw [str_ascii_app, IH]
      rfl
  · intro head args IHhead IHargs
    rw [render_app]
    have Ha := render_args_ascii_of args IHargs
    rw [str_ascii_app, str_ascii_app, str_ascii_app, Ha]
    cases expr_is_unary head with
    | true =>
      dsimp only [cond]
      rw [str_ascii_app, str_ascii_app, IHhead]
      rfl
    | false =>
      dsimp only [cond]
      rw [IHhead]
...
theorem render_args_ascii : ∀ es, Names.str_ascii (render_args es) = true :=
  fun es => render_args_ascii_of es (fun e _ => render_expr_ascii e)
...
theorem render_ne_exprs_ascii : ∀ es, Names.str_ascii (render_ne_exprs es) = true :=
  fun es => render_args_ascii (Collections.ne_to_list es)
...
theorem render_const_spec_ascii : ∀ s, Names.str_ascii (render_const_spec s) = true := by
  intro s
  ...
  | ExplicitConstInit ty vs =>
    ...
    rw [str_ascii_app, str_ascii_app, render_opt_type_ascii, render_ne_exprs_ascii vs]
    rfl
  | InheritedConstInit => rfl

theorem render_var_spec_ascii : ∀ s, Names.str_ascii (render_var_spec s) = true := by
  intro s
...
theorem render_spec_lines_ascii {A : Type} (render : A → Str) (depth : Nat) :
    ...
    ∀ specs, Names.str_ascii (render_spec_lines render depth specs) = true := by
  intro Hr specs
  induction specs with
  | nil => rfl
  | cons s rest IH =>
    show Names.str_ascii (indent depth ++ render s ++ nl ++ render_spec_lines render depth rest) = true
    rw [str_ascii_app, str_ascii_app, str_ascii_app, indent_ascii, Hr s, IH]
    rfl

theorem render_group_ascii {A : Type} (kw : Str) (render : A → Str) (depth : Nat) :
    Names.str_ascii kw = true →
    (∀ s, Names.str_ascii (render s) = true) →
    ∀ specs, Names.str_ascii (render_group kw render depth specs) = true := by
  intro Hkw Hr specs
  cases specs with
  | nil =>
    ...
    rw [str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, Hkw,
      render_spec_lines_ascii render (depth + 1) Hr [], indent_ascii]
    decide
  | cons s rest =>
    cases rest with
    | nil =>
      show Names.str_ascii (kw ++ str! " " ++ render s) = true
      rw [str_ascii_app, str_ascii_app, Hkw, Hr s]
      decide
    | cons s2 rest' =>
      show Names.str_ascii (kw ++ str! " (" ++ nl ++ render_spec_lines render (depth + 1) (s :: s2 :: rest')
        ++ indent depth ++ str! ")") = true
      rw [str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, Hkw,
        render_spec_lines_ascii render (depth + 1) Hr (s :: s2 :: rest'), indent_ascii]
      decide

theorem render_declaration_ascii : ∀ depth d, Names.str_ascii (render_declaration depth d) = true := by
  intro depth d
  cases d with
  | ConstDecl specs => exact render_group_ascii _ _ depth (by decide) render_const_spec_ascii specs
  | VarDecl specs => exact render_group_ascii _ _ depth (by decide) render_var_spec_ascii specs
  | TypeDecl specs => exact render_group_ascii _ _ depth (by decide) render_type_spec_ascii specs

theorem render_stmt_ascii : ∀ depth s, Names.str_ascii (render_stmt depth s) = true := by
  intro depth s
  cases s with
  | ExprStmt e =>
    show Names.str_ascii (indent depth ++ render_expr e ++ nl) = true
    rw [str_ascii_app, str_ascii_app, indent_ascii, render_expr_ascii e]
    decide
  | DeclarationStmt d =>
    show Names.str_ascii (indent depth ++ render_declaration depth d ++ nl) = true
    rw [str_ascii_app, str_ascii_app, indent_ascii, render_declaration_ascii depth d]
    decide
  | ShortVarDecl names vs =>
    ...
    rw [str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, indent_ascii, render_ne_names_ascii,
      render_ne_exprs_ascii vs]
    decide

theorem render_stmts_ascii : ∀ depth ss, Names.str_ascii (render_stmts depth ss) = true := by
  intro depth ss
  induction ss with
  | nil => rfl
  | cons s ss' IH =>
    show Names.str_ascii (render_stmt depth s ++ render_stmts depth ss') = true
    rw [str_ascii_app, render_stmt_ascii depth s, IH]
    rfl

theorem render_block_ascii : ∀ depth b, Names.str_ascii (render_block depth b) = true := by
  intro depth b
  cases b with
  | MakeBlock ss => exact render_stmts_ascii depth ss

theorem render_top_level_decl_ascii : ∀ t, Names.str_ascii (render_top_level_decl t) = true := by
  intro t
  cases t with
  | TopDeclaration d =>
    show Names.str_ascii (render_declaration 0 d ++ nl) = true
    rw [str_ascii_app, render_declaration_ascii 0 d]
    decide
  | Main body =>
    show Names.str_ascii (str! "func main() {" ++ nl ++ render_block 1 body ++ str! "}" ++ nl) = true
    rw [str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, render_block_ascii 1 body]
    decide

theorem render_top_levels_ascii : ∀ ts, Names.str_ascii (render_top_levels ts) = true := by
  intro ts
  induction ts with
  | nil => rfl
  | cons t ts' IH =>
    show Names.str_ascii (nl ++ render_top_level_decl t ++ render_top_levels ts') = true
    rw [str_ascii_app, str_ascii_app, render_top_level_decl_ascii t, IH]
    decide

theorem imports_ascii : ∀ xs, Names.str_ascii (imports xs) = true := by
  intro xs; rw [imports_nil_bytes]; rfl

theorem file_ascii : ∀ f, Names.str_ascii (file f) = true := by
  intro f
  unfold file
  rw [str_ascii_app, str_ascii_cons, str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app, str_ascii_app,
    render_top_levels_ascii, imports_ascii]
...
def ascii_digit (c : UInt8) : Nat := c.toNat - 48
...
def decode_string_body : Str → Option Str
  ...
  | c :: rest =>
      bif decide (c = dquote_c) then                            -- closing quote
        (match rest with | [] => some [] | _ :: _ => none)      -- trailing bytes ⇒ reject
      else bif decide (c = bslash_c) then                       -- an escape
        (match rest with
         | [] => none
         | e :: rest2 =>
             bif decide (e = dquote_c) then Option.map (dquote_c :: ·) (decode_string_body rest2)
             else bif decide (e = bslash_c) then Option.map (bslash_c :: ·) (decode_string_body rest2)
             else bif decide (e = byte! 'n') then Option.map (nl_c :: ·) (decode_string_body rest2)
             else bif decide (e = byte! 't') then Option.map (tab_c :: ·) (decode_string_body rest2)
             else bif decide (e = byte! 'r') then Option.map (cr_c :: ·) (decode_string_body rest2)
             else bif decide (e = byte! 'x') then
               (match rest2 with
                | h1 :: h2 :: rest3 =>
                    (match decode_hex_digit h1, decode_hex_digit h2 with
                     | some v1, some v2 =>
                         Option.map (UInt8.ofNat (v1 * 16 + v2) :: ·) (decode_string_body rest3)
...
def decode_string_literal (s : Str) : Option Str :=
  match s with
  | c :: rest => bif decide (c = dquote_c) then decode_string_body rest else none
  | [] => none
...
theorem byte_reconstruct : ∀ c : UInt8, UInt8.ofNat (c.toNat / 16 * 16 + c.toNat % 16) = c := by
  intro c
...
private theorem decode_hex_case : ∀ h1 h2 rest3,
    decode_string_body (bslash_c :: byte! 'x' :: h1 :: h2 :: rest3)
    = match decode_hex_digit h1, decode_hex_digit h2 with
      | some v1, some v2 => Option.map (UInt8.ofNat (v1 * 16 + v2) :: ·) (decode_string_body rest3)
      | some _, none => none
      | none, some _ => none
...
theorem decode_hex_prefix : ∀ hi lo tail,
    hi < 16 → lo < 16 →
    decode_string_body (bslash_c :: byte! 'x' :: hex_digit hi :: hex_digit lo :: tail)
    = Option.map (UInt8.ofNat (hi * 16 + lo) :: ·) (decode_string_body tail) := by
  intro hi lo tail Hhi Hlo
  rw [decode_hex_case, hex_digit_decode hi Hhi, hex_digit_decode lo Hlo]

theorem hex_escape_exact : ∀ c tail,
    decode_string_body (hex_escape c ++ tail) = Option.map (c :: ·) (decode_string_body tail) := by
  intro c tail
  have Hb := nat_of_ascii_lt_256 c
  have Hhi : c.toNat / 16 < 16 := (Nat.div_lt_iff_lt_mul (by decide)).2 Hb
  have Hlo : c.toNat % 16 < 16 := Nat.mod_lt _ (by decide)
  show decode_string_body (bslash_c :: byte! 'x' :: hex_digit (c.toNat / 16) :: hex_digit (c.toNat % 16) :: tail)
...
private theorem byte_of_toNat : ∀ (c : UInt8) (n : Nat), c.toNat = n → c = UInt8.ofNat n :=
  fun c _ h => (UInt8.ofNat_toNat : UInt8.ofNat c.toNat = c).symm.trans (congrArg UInt8.ofNat h)
...
private theorem decode_simple_escape : ∀ (e d : UInt8) tail,
    (bif decide (e = dquote_c) then some dquote_c else bif decide (e = bslash_c) then some bslash_c
     else bif decide (e = byte! 'n') then some nl_c else bif decide (e = byte! 't') then some tab_c
     else bif decide (e = byte! 'r') then some cr_c else none) = some d →
    decode_string_body (bslash_c :: e :: tail) = Option.map (d :: ·) (decode_string_body tail) := by
  intro e d tail H
  rw [decode_string_body.eq_def]
  dsimp only
  rw [show decide (bslash_c = dquote_c) = false from rfl, show decide (bslash_c = bslash_c) = true from rfl]
  dsimp only [cond]
  revert H
  cases decide (e = dquote_c) with
  | true => intro H; dsimp only [cond] at H ⊢; rw [← Option.some.inj H]
  | false =>
  cases decide (e = bslash_c) with
  | true => intro H; dsimp only [cond] at H ⊢; rw [← Option.some.inj H]
  | false =>
  cases decide (e = byte! 'n') with
  | true => intro H; dsimp only [cond] at H ⊢; rw [← Option.some.inj H]
  | false =>
  cases decide (e = byte! 't') with
  | true => intro H; dsimp only [cond] at H ⊢; rw [← Option.some.inj H]
  | false =>
  cases decide (e = byte! 'r') with
  | true => intro H; dsimp only [cond] at H ⊢; rw [← Option.some.inj H]
  | false => intro H; dsimp only [cond] at H; exact nomatch H

theorem decode_render_byte : ∀ c tail,
  ...
  | true =>
    rw [byte_of_toNat c 34 (of_decide_eq_true E34)]
  ...
  | true =>
    rw [byte_of_toNat c 92 (of_decide_eq_true E92)]
  ...
  | true =>
    rw [byte_of_toNat c 10 (of_decide_eq_true E10)]
    show decode_string_body (bslash_c :: byte! 'n' :: tail) = Option.map (nl_c :: ·) (decode_string_body tail)
    exact decode_simple_escape _ _ tail rfl
  | false =>
  cases E9 : decide (c.toNat = 9) with
  | true =>
    rw [byte_of_toNat c 9 (of_decide_eq_true E9)]
    show decode_string_body (bslash_c :: byte! 't' :: tail) = Option.map (tab_c :: ·) (decode_string_body tail)
    exact decode_simple_escape _ _ tail rfl
  | false =>
  cases E13 : decide (c.toNat = 13) with
  | true =>
    rw [byte_of_toNat c 13 (of_decide_eq_true E13)]
    show decode_string_body (bslash_c :: byte! 'r' :: tail) = Option.map (cr_c :: ·) (decode_string_body tail)
    exact decode_simple_escape _ _ tail rfl
  | false =>
  cases Hp : (decide (32 ≤ c.toNat) && decide (c.toNat ≤ 126)) with
  | true =>
    have Hnd : decide (c = dquote_c) = false := by
      cases hq : decide (c = dquote_c) with
      | false => rfl
      | true =>
        have hc : c = dquote_c := of_decide_eq_true hq
        subst hc
        exact absurd E34 (by decide)
    have Hnb : decide (c = bslash_c) = false := by
      cases hq : decide (c = bslash_c) with
      | false => rfl
      | true =>
    ...
    rw [Hnd, Hnb, Hp]
    dsimp only [cond]
  | false => exact hex_escape_exact c tail

theorem decode_body_render : ∀ s,
  ...
  | cons c s' IH =>
    ...
    rw [← str_app_assoc, decode_render_byte c, IH]
    rfl

theorem string_roundtrip : ∀ s, decode_string_literal (string_literal s) = some s := by
  intro s
  show decode_string_body (string_body s ++ dquote_c :: []) = some s
  exact decode_body_render s

/-! The literal is quote-delimited and holds no raw newline or carriage return. -/
def byte_not_nl_cr (c : UInt8) : Bool :=
...
theorem hex_digit_not_nl_cr : ∀ k, k < 16 → byte_not_nl_cr (hex_digit k) = true := by decide
...
theorem hex_escape_no_nl_cr : ∀ c, str_no_nl_cr (hex_escape c) = true := by
  ...
  have Hhi : c.toNat / 16 < 16 := (Nat.div_lt_iff_lt_mul (by decide)).2 Hb
  have Hlo : c.toNat % 16 < 16 := Nat.mod_lt _ (by decide)
  show (byte_not_nl_cr bslash_c && (byte_not_nl_cr (byte! 'x')
    && (byte_not_nl_cr (hex_digit (c.toNat / 16)) && (byte_not_nl_cr (hex_digit (c.toNat % 16)) && true)))) = true
  rw [hex_digit_not_nl_cr _ Hhi, hex_digit_not_nl_cr _ Hlo]
  decide
...
theorem byte_escape_quote : string_byte (UInt8.ofNat 34) = bslash_c :: dquote_c :: [] := by decide
theorem byte_escape_backslash : string_byte (UInt8.ofNat 92) = bslash_c :: bslash_c :: [] := by decide
theorem byte_escape_nl : string_byte (UInt8.ofNat 10) = bslash_c :: byte! 'n' :: [] := by decide
theorem byte_escape_tab : string_byte (UInt8.ofNat 9) = bslash_c :: byte! 't' :: [] := by decide
theorem byte_escape_cr : string_byte (UInt8.ofNat 13) = bslash_c :: byte! 'r' :: [] := by decide
theorem byte_escape_nul : string_byte (UInt8.ofNat 0)
    = bslash_c :: byte! 'x' :: byte! '0' :: byte! '0' :: [] := by decide
theorem byte_escape_del : string_byte (UInt8.ofNat 127)
    = bslash_c :: byte! 'x' :: byte! '7' :: byte! 'f' :: [] := by decide
theorem byte_escape_80 : string_byte (UInt8.ofNat 128)
    = bslash_c :: byte! 'x' :: byte! '8' :: byte! '0' :: [] := by decide
theorem byte_escape_ff : string_byte (UInt8.ofNat 255)
    = bslash_c :: byte! 'x' :: byte! 'f' :: byte! 'f' :: [] := by decide
theorem literal_empty : string_literal (str! "") = dquote_c :: dquote_c :: [] := by decide
theorem literal_ascii : string_literal (str! "hi")
    = dquote_c :: byte! 'h' :: byte! 'i' :: dquote_c :: [] := by decide

/-! The render-time constant-status authority speaks the vocabulary Compilable.TypeResolution owns. -/
def digit_value (c : UInt8) : Option Nat :=
  let n := c.toNat
...
def read_signed_dec (s : Str) : Option (Int × Str) :=
  ...
  | [] => none
  | c :: s' =>
      bif decide (c = byte! '-') then (let (m, r) := read_nat s' 0; some (-m, r))
      else bif decide (c = byte! '+') then (let (m, r) := read_nat s' 0; some (m, r))
      else (let (m, r) := read_nat (c :: s') 0; some (m, r))

def decode_decimal_body (s : Str) : Option Float.Constant :=
  match read_signed_dec s with
  | some (coeff, a :: b :: c :: r2) =>
      bif decide (a = byte! '.') && (decide (b = byte! '0') && decide (c = byte! 'e')) then
...
theorem char_digit_not : ∀ c d,
    digit_value c = some d → decide (c = byte! '-') = false ∧ decide (c = byte! '+') = false := by
  intro c d Hc
  constructor
  · cases E : decide (c = byte! '-') with
    | false => rfl
    | true =>
      have hc : c = byte! '-' := of_decide_eq_true E
      subst hc
...
theorem read_signed_dec_sign : ∀ sgn mag rest,
    (sgn = byte! '-' ∨ sgn = byte! '+') → str_all_digits mag = true → head_not_digit rest →
    read_signed_dec (sgn :: (mag ++ rest))
    = some ((bif decide (sgn = byte! '-') then -(dval0 mag) else dval0 mag), rest) := by
...
theorem read_signed_dec_render_signed_Z : ∀ z rest, head_not_digit rest →
  ...
  | true =>
    ...
    show read_signed_dec (byte! '-' :: (Decimal.integer (-z) ++ rest)) = some (z, rest)
    rw [read_signed_dec_sign (byte! '-') _ rest (Or.inl rfl) (str_all_digits_print_Z (-z) (by omega)) Hrest]
...
theorem read_signed_dec_render_signed_exp : ∀ z, read_signed_dec (signed_exp z) = some (z, str! "") := by
  ...
  | true =>
    have Hlt' : z < 0 := of_decide_eq_true Hlt
    show read_signed_dec (byte! '-' :: Decimal.integer (-z)) = some (z, [])
    rw [← str_app_nil (Decimal.integer (-z)),
      read_signed_dec_sign (byte! '-') _ (str! "") (Or.inl rfl) (str_all_digits_print_Z (-z) (by omega)) True.intro]
  ...
  | false =>
    ...
    show read_signed_dec (byte! '+' :: Decimal.integer z) = some (z, [])
    rw [← str_app_nil (Decimal.integer z),
      read_signed_dec_sign (byte! '+') _ (str! "") (Or.inr rfl) (str_all_digits_print_Z z Hge) True.intro]
    show some (dval0 (Decimal.integer z), []) = some (z, [])
...
theorem go_int_lit_neg : ∀ s,
    str_all_digits s = true → s ≠ str! "" → go_int_lit (byte! '-' :: s) = true := by
  intro s Hdig Hne
  cases s with
  | nil => exact absurd rfl Hne
  | cons c s' => exact Hdig
...
theorem go_int_lit_negated_integer_literal : ∀ n : Nat,
    go_int_lit (byte! '-' :: Decimal.integer (n : Int)) = true :=
...
private theorem decode_decimal_body_shape : ∀ coeff r2 s,
    read_signed_dec s = some (coeff, byte! '.' :: byte! '0' :: byte! 'e' :: r2) →
...
theorem decode_render_decimal : ∀ d, decode_decimal (decimal d) = some (Float.decimal_value d) := by
  ...
  | false =>
    have Hbody : decode_decimal_body (signed_Z (Float.coefficient d) ++ str! ".0e" ++ signed_exp (Float.exponent d))
      ...
      rw [← str_app_assoc]
      show decode_decimal_body (signed_Z (Float.coefficient d)
          ++ (byte! '.' :: byte! '0' :: byte! 'e' :: signed_exp (Float.exponent d)))
        = _
      rw [decode_decimal_body_shape _ _ _
          (read_signed_dec_render_signed_Z (Float.coefficient d)
            (byte! '.' :: byte! '0' :: byte! 'e' :: signed_exp (Float.exponent d)) (head_not_digit_dot0e _)),
...
private theorem digit_ne_minus : ∀ a, a < 10 → decide (Decimal.digit a = byte! '-') = false := by decide

theorem positive_head_not_minus : ∀ p, 0 < p →
    match Decimal.positive p with | c :: _ => decide (c = byte! '-') = false | [] => False := by
  intro p Hp
  unfold Decimal.positive
  rcases List.eq_nil_or_concat (Decimal.positive_digits 10 p) with Hnil | ⟨init, a, Ha⟩
  · exact absurd Hnil (Decimal.positive_digits_nonnil 10 p Hp)
...
theorem read_go_int_nonneg : ∀ z, 0 ≤ z → read_go_int (Decimal.integer z) = dval0 (Decimal.integer z) := by
  ...
  | .ofNat (n + 1), _ =>
    ...
    | cons c s' =>
      intro Hh
      have Hh' : decide (c = byte! '-') = false := Hh
      show (bif decide (c = byte! '-') then -(dval0 s') else dval0 (c :: s')) = dval0 (c :: s')
      rw [Hh']
      rfl
  | .negSucc _, H => exact absurd H (by omega)
