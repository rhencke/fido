# Active task: the C4 canonical candidate-bound performance evidence derivation candidate
...
- **The active candidate is the canonical candidate-bound performance evidence derivation; exact semantic
  outputs are unchanged.** One-DAG remains implemented and operationally successful — one Dune build and one
  project verification solve. Every performance measurement input is candidate-bound: each basis in
  `.review/perf/performance-bases.tsv` is digest-verified against its exact source commit (exactly one
  CURRENT basis; older bases stay registered HISTORICAL). One accounting engine, `tools/perf-work-span.py`,
  computes aggregate task elapsed (work), the real predecessor longest-path critical span, the separate
  descriptive interval union, and per-worker reachability from the committed normalized event table, whose
  registered topology grammar rejects any omitted required event; raw attribution
  (`tools/witness-profile-attribution.py`) performs profile classification only and no reachability
  arithmetic. The work/span and reachability summaries are generated by that one engine and byte-compared
  against the committed `.review/perf/verification-dag-work-span.tsv` and
  `.review/perf/verification-dag-reachability.tsv` by both `make perf-evidence` and the staged pre-commit
  hook — the exact current values live in those generated tables, never in prose. The theorem-backed
  evidence observation is **not** implemented (`realistic_expected_saving = UNKNOWN_PENDING_SPIKE`; a
  measurement-only spike precedes any implementation contract), and local declared-and-used analysis remains
  the next semantic root. C4 remains implemented but not accepted or complete.
