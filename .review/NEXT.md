...
- **The active candidate is fixture-performance repair; exact semantic outputs are unchanged.** The prior
  fast-disposition hypothesis was measured and rejected (all 76 verdict-only fixtures cost 1.24s of
  WitnessReject's 40.28s; a pure disposition proof reduces in ~8ms); the real measured hotspot was the
  exact-evidence fixture matrix compiling in one serial unit. The candidate splits it into a shared prelude
  plus four cost-balanced chunks compiled in one bounded four-slot schedule (chunk wall measured 12s vs the
  ~41s serial entry), proves the `p_two_iota` cluster from one shared let-bound observation (8.88s → 3.20s,
  statements unchanged, projections VM-free), and changes no disposition, diagnostic, issue, view, golden or
  generated byte — the tracked inventory manifest and the audit's derived chunk root set keep every fixture
  name resolvable and every chunk audited fail-closed. Performance evidence is now honest by construction:
  required scenarios need successful complete rows, a baseline-only file is not current evidence, newly
  published measurements must be bound to the exact basis they were measured on, and the staged pre-commit
  hook validates the staged evidence with the staged validator and digest algorithm. Local declared-and-used
  analysis remains the next semantic root.
