...
- **M1 SOURCE DIET IS THE SOLE ACTIVE WORK.**
  ...
  M1 removes every byte with no present, load-bearing purpose: stale comments, source archaeology,
  superseded prose, dead files, and dead declarations. It changes no surviving declaration's name, type,
  statement or proof body, and no Go meaning, program set, diagnostic, trust boundary or generated byte.
- **M1 candidate offered for human review:** `7dc9ff3bb3450cc3bcc41abfb7c5c24154967f3d`. All fifteen
  obligations are closed. Only Rob accepts M1.
...
- **M1 result — do NOT regress.** Every `.v` comment is one physical line, at most 120 characters, one
  sentence, one current local fact, with no archaeology and no split-paragraph evasion; the exception ledger
  is EMPTY and staying that way is the goal, not a quota. No surviving Rocq code changed — `make diet` proves
  it by comparing comment-stripped token streams against the sealed baseline ref and requiring the vanished
  declarations to equal the deletion ledger exactly. Generated bytes, reviewed goldens and every gate result
  are unchanged. Three scoping calls are recorded in `.review/OPEN_QUESTIONS.md` with defaults rather than
  guessed.
