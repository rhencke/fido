...
scope decision: **`Index.Program` is NOT deleted during repair 14** — if the retained core makes it redundant,
...
⚠ **Green commands did not catch either of the last two blockers.** A `List.find` keyed by `Index.Key` typechecks,
proves, builds and emits identical bytes — that was repair 13. A `Compilable.Program` that keeps copied
