...
- Status: **ROOT in progress** — `GoIndex.v` is being grown bottom-up in fully-proven, always-green commits
  (each keeps `make check`/pre-commit GREEN and generated bytes byte-identical).  Landed so far:
  ...
  - Gate 461/461 axiom-free; whole-theory audit covers GoIndex; generated bytes byte-identical throughout.
  - **Remaining before the ROOT barrier**: Pillar 3 — the snapshot-indexed **sealed reference layer over
    `GoProgram`** (`SyntaxIndex`/`FileRef`/`NodeRef`/`NodeKey` + kind-refined `NodeRefOf` refs; validated
    minting; the total query API; parent/children/ancestry lifted through references; the exact
    source-occurrence correspondence lifted through the sealed API), the §19 **indexed traversal** (original
    syntax fragment + canonical reference together), and the §21/§22 **snapshot-locality + mutation-sensitive
    fixtures** over the real grammar (incl. `println(1, 1)` distinct-ref).  Then the ROOT commit
    `milestone(root): C2 —` and the ROOT Codex barrier.  (OccurrenceSpike deletion + the minimal
    compiler/type integration are FINAL-stage, after ROOT-GREEN.)
