...
- Status: **C3 FINAL barrier — all defects from ALL FIVE FINAL reviews CLOSED @`bced8c0`; one-pass hot path +
  end-to-end diagnostic soundness (same-file/nearest-first/NoDup nested scar + dup precedence/distinctness) +
  cross-snapshot determinism + the COMPLETE EXACT §22.1-22.16/§23 structured-diagnostic & fact-query fixture
  suite + clean GoIndex/GoTypes/GoCompile dependency ownership (GoTypes imports no GoIndex) all proved + gated
  (530/530); `make check` green on the working tree; generated bytes byte-identical throughout.  Awaiting the
