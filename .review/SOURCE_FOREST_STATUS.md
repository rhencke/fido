...
- Active checkpoint: **C1A** (Standard Collection Foundations and Repository-Wide Collection Audit — a binding
  FOUNDATIONAL correction checkpoint inside the campaign; C0/C0A/C0B and C1 all accepted; C2 remains forbidden
  until C1A is Codex-green, pushed, and human-approved).  The binding completion contract is
  `.review/COLLECTION_FOUNDATIONS_MASTER_PLAN.md` (= `.review/NEXT_STEPS.md`, installed verbatim).
...
- Root Codex result / repair SHAs: pending (awaiting ROOT Codex review).
- Final candidate SHA: pending (FINAL follows directly after ROOT green: §7–§9 order-independence/extensionality
  theorems, OCaml `Map`/`Set` migration in the sink + `GlobRef.Set` audit roots, sink/e2e dup-path + permutation
  fixtures, docs reconciliation, PAINFUL_LESSONS #14).
- Final Codex result / repair SHAs: pending
- Verification result (`make check`): **ROOT — green on the working tree** (proof + e2e + byte-compare).
- Push result: pending
