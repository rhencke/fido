...
- Active checkpoint: **C1** (specification-shaped file roots and path-keyed source forest; C0/C0A/C0B accepted;
  C2 remains forbidden until C1 is human-approved)
...
- Final Codex result / repair SHAs: first review of `a5e9895` BLOCKED (review `task-mroizvrb`,
  ...
  check GREEN; generated bytes UNCHANGED.
...
- `GoRender.v`: `render_package_clause` renders the SOURCE-owned package clause (`PkgMain` → `main`);
  ...
  render_file`; deleted `sp_pkg_name_main`.  Witnesses migrated (`singleton_program`/`build_program` take
  declaration lists; `mkCompilable X valid`).
