...
- Status: **C3 FINAL barrier — all defects from ALL SEVEN FINAL reviews CLOSED @`29c32c5`; one-pass hot path +
  end-to-end diagnostic soundness + cross-snapshot determinism + the COMPLETE EXACT §22.1-22.16/§23 fixture
  suite + §16 NodeKeyMap-canonical diagnostic order (mixed node-primary ordering) + clean
  GoIndex/GoTypes/GoCompile dependency ownership all proved + gated (532/532); `make check` green on the working
  tree; generated bytes byte-identical throughout.  Awaiting the FINAL Codex re-review.**
