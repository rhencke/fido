...
- Active checkpoint: **C2** (Production Occurrence Index, Snapshot-Local References, and Indexed Traversal —
  the one production structural module `GoIndex.v`).  C0/C0A/C0B, C1, C1A, **and C1B all accepted/Codex-green
  and human-accepted**; C2 is NOW AUTHORIZED by Rob.  C3 remains FORBIDDEN until Rob reviews and explicitly
  approves C2.  The binding completion contract is `.review/C2_PRODUCTION_GOINDEX_PLAN.md`
  (= `.review/NEXT_STEPS.md`, installed verbatim).
...
- Status: **§25 FINAL complete — awaiting the FINAL Codex barrier** (ROOT is Codex-GREEN) — `GoIndex.v` was grown bottom-up in fully-proven,
  ...
  - **§25 FINAL** (after ROOT-GREEN, no human stop) — COMPLETE:
    ...
    - **§19 indexed traversal** (`milestone(wip)` `d53ad70`): the structural `occs_file` fold + interval bounds
      + `occs_file_sound`/`_complete`/`_exact`/`_sorted`/`_nodup`; the ref-level `Snap.visit_file` (each
      validated `NodeRef` paired with its ORIGINAL `SourceOccurrence` in ONE pass) with `visit_file_view`/
      `_complete`/`_order`/`_nodup` and `visit_file_snd` (the projection IS `occs_file`).
    - **§20/§25 minimal integration**: the GoTypes typing layer now consumes the index — `indexed_program_typedb`
      let-binds ONE `Snap.index_program p`, threads it through `visit_file`, takes each per-occurrence decision
      through the TOTAL query API (`node_role`/`node_at`) delegating to the SAME `expr_typedb`/`const_info`
      resolver, and is proved EQUAL to the existing `program_typedb` (`indexed_program_typedb_eq`); the
      per-file `indexed_source_file_typedb_eq` and `occs_file_typedb_eq` support it.  No semantic kernel is
      modified, so `ProgramTyped`/`ProgValid`/`go_compile`/package summaries/GoSafe/GoRender/GoEmit are
      untouched and every generated byte is unchanged; GoAST gains one reusable `file_bindings_find` bridge.
    ...
    - Then `milestone(final): C2 —` and the FINAL Codex barrier.
