...
- **Candidate `89b8e54` = C4 Implementation Review BLOCKING; authorized repair `C4-retained-facts-and-diagnostics-repair-1` IN PROGRESS.** The blocking classes (repair authority `.review/C4_IMPLEMENTATION_REPAIR_1.md`):
  - **2.1** the type-name fact table is NOT on the production semantic path — production typing/diagnostics call
    `predeclared_type` inline; `elaborate_indexed` rebuilds the table via a second `prog_type_name_fact_table p ip`
    after the decision instead of sealing the map production used;
  - **2.2** the production expression path keeps a `NodeKey -> option ConstInfo` status map THEN copies into a
    separate ExprFact map — not ONE expression-outcome/fact authority;
  - **2.3** `DRInvalidConversion`/`ErasedDiagnostic` omit the target `TypeNameRef` and lose the source spelling
    (invalid `byte(...)` and `uint8(...)` erase to the same payload; same for `rune`/`int32`);
  - **2.4** the pinned-Go alias differential covers only ACCEPTED `byte(255)`/`rune(65)` — no REJECTED alias scars;
  - **2.5** permanent prose (ARCHITECTURE, master-plan) still names `EIntConvert`/`EFloatConvert`/`EComplexConvert`
    as current + the dead `integer_keyword`/`float_keyword`/`complex_keyword` renderer helpers linger;
  - **2.6** the freeze process (`review(final)` + §19.16–19.20) was not completed.
  ...
  Repair target architecture (§3): ONE retained `visit`; ONE once-built type-name map consumed by production +
  sealed (no rebuild); production obtains the target `TypeNameRef` through the retained index, reads the
  `TypeNameFact` + operand `ExprFact`, calls `convert_const` ONCE, stores success or exact local invalid-conversion
  evidence; diagnostics consume the stored outcome (no second `convert_const`/resolver call); the typed reason +
  erased report carry the target `TypeNameRef` + source spelling; the full pinned-Go accept/reject alias matrix.

  Superseded record of the prior (pre-repair) implementation:
- ~~**Implementation COMPLETE**~~ (superseded by the BLOCKING result above — kept for the landed inventory):
