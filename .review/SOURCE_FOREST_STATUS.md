...
- Status: **C3 FINAL barrier — all defects from ALL THREE FINAL reviews CLOSED @`719a005`; one-pass hot path +
  end-to-end diagnostic soundness (with same-file/nearest-first/NoDup nested scar + dup precedence/distinctness)
  + cross-snapshot determinism + concrete §22/§23 structured-diagnostic & fact-query fixtures all proved + gated
  (519/519); `make check` green on the working tree; generated bytes byte-identical throughout.  Awaiting the
