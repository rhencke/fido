...
- ROOT candidate SHA: **`6847cc4`** (`milestone(root): C2 — canonical production occurrence index and
  navigation`).  Awaiting the ROOT Codex barrier (§24).  Repairs: `review(root): C2 —`.  After ROOT-GREEN,
  continue directly to §25 FINAL integration (no human stop between ROOT and FINAL).
- Status: **ROOT candidate pushed** — `GoIndex.v` was grown bottom-up in fully-proven, always-green commits
  (each kept `make check`/pre-commit GREEN and generated bytes byte-identical); gate 520/520 axiom-free.
  Landed:
  ...
  - Gate 520/520 axiom-free; whole-theory audit covers GoIndex; generated bytes byte-identical throughout.
  - **Deferred to §25 FINAL (after ROOT-GREEN, no human stop)**: the §19 **indexed traversal** (original
    syntax fragment + canonical reference together), the minimal let-bound-index compiler/type integration
    with semantic + byte preservation, **`OccurrenceSpike.v` deletion** (once `GoIndex` subsumes its accepted
    surfaces), and the docs/gate/audit reconciliation.
