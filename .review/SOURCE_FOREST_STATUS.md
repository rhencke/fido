...
- Status: **§25 FINAL in progress** (ROOT is Codex-GREEN) — `GoIndex.v` was grown bottom-up in fully-proven,
  ...
  - **§25 FINAL progress** (after ROOT-GREEN, no human stop): **`OccurrenceSpike.v` DELETED** — removed from
    `dune`, the gate, and the repo; `GoIndex` subsumes all its accepted C0/C0A/C0B surfaces over the real
    grammar (no parallel spike authority remains); gate now 437/437 axiom-free, coverage OK, bytes
    byte-identical; docs reconciled (CLAUDE / PROGRESS / COLLECTION_AUDIT).  **Remaining before the FINAL
    barrier**: the §19 **indexed traversal** (original syntax fragment + canonical reference together), the
    minimal let-bound-index integration with semantic + byte preservation, and the ARCHITECTURE/master-plan
    reconciliation.  Then `milestone(final): C2 —` and the FINAL Codex barrier.
