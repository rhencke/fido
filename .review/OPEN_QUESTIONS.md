...
## OQ-M2-03 — the retained kind is `wall_elapsed`, and Repair 4 §5 spells it `direct_wall_elapsed`

**Owner:** reviewer. **Blocks M2:** no. **Default if unanswered:** keep `wall_elapsed`, because the other name
would be false.

Repair 4 §5 deletes the summed projection and lists four kinds to retain, the first written
`direct_wall_elapsed`. The implementation emits `wall_elapsed`, and the difference is not cosmetic: that kind
is carried by hook-stage children as well as by direct executions. In the recorded observation
`precommit.builder`, `precommit.export-index`, `precommit.pytools` and every other pre-commit stage carry
`wall_elapsed` while being measured INSIDE `precommit.full`. Renaming the constant would put the word DIRECT on
roughly twenty-five intervals per pre-commit trace that are not direct executions of anything.

So the contract now names the kind the implementation actually emits, rather than adopting a name that makes a
false claim about most of its members. The substantive instruction — delete the projection, retain the other
four — is applied in full.

There is a real question underneath, and it belongs to the reviewer rather than to me. A pre-commit stage IS
contained: it is an interval measured inside a parent by a checkpoint pair, which is exactly what
`contained_wall_elapsed` means. It carries `wall_elapsed` only because containment is currently decided by the
`contained_in` relation over PUBLIC commands, and hook stages are `derived` instead. If the reviewer wants the
vocabulary to match the concept, the honest change is to make hook-stage children contained rather than to
rename the direct kind — and that would move about twenty-five metric identities per trace, so it is a change
with its own evidence, not a word swap.
