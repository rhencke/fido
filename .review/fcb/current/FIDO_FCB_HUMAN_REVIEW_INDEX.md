...
|---|---|---|---|---|
| `C4-REVIEW` | **BLOCKED — awaiting the repair-16 candidate** | Candidate `deda8bd91dbfebf75895c8786732a4ed9d7952f2` is BLOCKING, the seventeenth to block; `25bcd7aa6b53f1e506a32c5077990a884bea8574` is its documentation-only freeze, not a separate candidate. Repair 15 made substantial, real progress and did NOT close C4: package facts and diagnostics still began from a rerun of the program visit, `Compilable.Core` and `MakeCore` remained publicly constructible, the A005 naming gate could not see lower-case constructors and so reported a false green over live residue, and the direct capability fixtures still recovered their results through an equality to a rebuilt core. **Repair 16 is the sole active C4 work.** Review the next C4 candidate only after it is implemented and frozen. C4 is NOT accepted — only Rob accepts it. | `.review/C4_IMPLEMENTATION_REPAIR_16.md`, A001, A005, and the main review thread | C5, post-C4 features, the broad source cleanup and proof-module partitioning remain forbidden. |
