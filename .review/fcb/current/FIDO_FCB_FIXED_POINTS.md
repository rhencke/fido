...
**Amendment A008 reopens no fixed point.** It settles review scope and inserts the M0 Governance Closeout; it
changes no semantic contract, no protected projection, and no theorem or generated-byte guarantee. M0 itself
may change no Rocq definition, capability topology, program set, diagnostic result, transport role or
generated byte, so every component projection in this registry survives it unchanged.

**Amendment A007 reopens no fixed point.** The post-C4 mechanical series it installs — M1 through M4 — is mechanical by definition: it may delete dead text and code, measure and reshape build dependencies, and refactor proof units and tooling, and it must preserve every parent fixed point and every component projection in this registry, along with Go meaning, the accepted and rejected program sets, diagnostic results, public correctness guarantees, trust boundaries and generated bytes. An M candidate that would move one of them is not mechanical and stops at that boundary.
