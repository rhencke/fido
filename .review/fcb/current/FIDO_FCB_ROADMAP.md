...
`M2 → M3 → Rob approves the exact M4 plan → M4 → checkpoint-definition Step 0 → C5`.

M3, M4 and C5 remain forbidden until Rob accepts M2.

## M0 — Governance Closeout (Governance D-29, amendment A008) — ACCEPTED

**Dependency:** C4 accepted.
**Contracts:** none consumed; no semantic contract changes.

One separately reviewed governance commit: publish the C4 disposition, retire repair-state archaeology to Git history, install the strict checkpoint-scope law, move current authority to the M-series, and verify the existing D-07 and D-24 state against the accepted snapshot. M0 is accepted under `M0-ACCEPT-86a63db`, and Git history owns its contract and its obligation matrix.
...
**Exit condition:** met — every obligation row closed with exact evidence, no functional file changed, the existing checks passed on the working tree and the staged snapshot, and Rob accepted M0.
