...
`M0 → M1 → M2 → M3 → Rob approves the exact M4 plan → M4 → checkpoint-definition Step 0 → C5`.

M1 through M4 and C5 remain forbidden until Rob accepts M0.
...
**Exit condition:** Every M0 obligation row closes with exact evidence; no functional file changes; the existing checks pass on the working tree and the staged snapshot; Rob accepts M0.
