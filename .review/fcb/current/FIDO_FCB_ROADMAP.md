...
**C4 is not accepted.** Amendments A001 through A007 are accepted. A001–A006 change no checkpoint assignment or dependency. A007 inserts the mechanical M-series between C4 acceptance closeout and C5 checkpoint-definition Step 0; it reassigns no closure row, latitude row or acceptance gate.
...
`the active C4 repair → human C4 review → C4 acceptance closeout → M1 → M2 → M3 → Rob approves the exact M4 plan → M4 → checkpoint-definition Step 0 → C5`.

C5 remains forbidden until Rob accepts C4. M1 through M4 are likewise forbidden until Rob accepts C4.
...
**Dependency:** Accepted C4 opaque static capability retaining the exact successful whole-elaboration object by construction, with total fact/layout/plan queries as projections and no rerun-based provenance; and M1 through M4 accepted, per Governance D-27.  
