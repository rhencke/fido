...
> **Supersedes:** `FIDO_FCB_GOVERNANCE_v2.md`  
...
- Every serious task resolves one exact repository ref, verifies `.review/fcb/current/FIDO_FCB_MANIFEST.sha256`, and never mixes FCB files across refs.
...
**Standing law:** `LAT-X004` is owned by `SPEC-096`, intrinsic `ExprFact`, and `SC-05`. FCB v3 retains the proved rounding-invariant accepted domain.
...
one bootstrap shim only. Root `CLAUDE.md` gives Claude Code the same bootstrap. A serious task uses one exact Git
ref, reads `.review/fcb/current/INDEX.md`, verifies the manifest, and consults the documents named by the current
Index. FCB bytes from different refs may not be mixed.
...
| ADR | FCB v3 status | Standing result | Reopen trigger |
|---|---|---|---|
