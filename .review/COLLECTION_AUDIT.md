...
| `GoCompile.prog_conv_outcomes` / `ExprFactTable` / `TypeNameFactTable` | occurrence -> `ExprOutcome` (the ONE expression-outcome authority) / `ExprFact` (a projection of its `EOOk`) / `TypeNameFact` | `NodeKey` (`mkKey FilePath positive`) | canonical (path then local id) | no (one entry per occurrence; a bucket value prevents overwrite) | by NodeKey | **`FMapAVL`** (`NodeKeyMapBase`, a thin `FMapAVL` wrapper over the `NodeKey` ordered key) | retained | the ONE bottom-up outcome map (`prog_conv_outcomes`; a conversion reads its target `TypeNameFact` + operand outcome, `convert_const` once) + the SEALED occurrence-keyed expression-fact table (a projection of it) + the once-built type-name fact table (sealed unchanged into `ElaborationFacts`); keys are validated occurrence NodeKeys (no foreign/forged key — `prog_expr_facts_domain` / `prog_type_name_facts_domain`); `elements` is a DERIVED canonical enumeration, never a second identity authority |
| `GoCompile.TypeNameFactTable` / `prog_type_name_facts` (§8) | type-name occurrence -> `TypeNameFact` (the resolved `GoType` ONLY) | `NodeKey` (`mkKey FilePath positive`) | canonical (path then local id) | no (one entry per type-name occurrence) | by NodeKey | **`FMapAVL`** (the SAME `NodeKeyMapBase` wrapper — no new collection) | retained | the SEALED occurrence-keyed type-name fact table (each conversion's source name resolved by `predeclared_type` to a `GoType`; fact stores the resolved type only, source identity stays in the `TypeNameRef`); domain = exactly the visited type-name occurrences (`prog_type_name_facts_domain` — no expression/foreign key); the total `type_name_fact_at` query PROJECTS the sealed table; retained in `ElaborationFacts`/`CompilableProgram` |
