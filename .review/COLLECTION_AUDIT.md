...
| `GoCompile.ExprWorkForest` (`ewf_blocks` / `ewf_items`) / `AnnotatedExprWorkForest` (`aewf_items`) | per-file + flat `ExprWork` / `WorkMember` enumerations (+ retained annotation contexts) | — | **source / per-file order** (semantic) | no (`ewf_keys_nodup` / `aewf_members` proved) | sequential fold / member handle | `list` | retained | proof-BACKED ORDERED views over the ONE source AST — the order is semantic for bottom-up outcome/fact/diagnostic/context processing, and key-`NoDup` is a proved theorem ABOUT a derived list, not identity storage; these are not maps |
| `GoCompile.OutcomeAccumulator` (`oa_map`) / `ForestOutcomeTable` (`fot_acc`) | `NodeKey` -> `ExprOutcome` (the ONE expression-outcome authority, over one exact suffix / the whole forest) | `NodeKey` (`mkKey FilePath positive`) | canonical (path then local id) | no (ONE entry per unique work key — key uniqueness + the `oa_domain` exact-domain fact prevent overwrite; **NO list bucket**) | by NodeKey | **`FMapAVL`** (`GoIndex.NodeKeyMapBase`, a thin `FMapAVL` wrapper over the `NodeKey` ordered key) | retained | identity-keyed TOTAL result lookup; exact domain = the retained forest / suffix members (`oa_covers` / `oa_domain`); `ForestOutcomeTable` pairs `fot_acc` with the exact `OutcomeTrace` (`fot_trace`) that built it; `elements` is a DERIVED enumeration, never a second identity authority |
| `GoCompile.OutcomeTrace` (`fot_trace`) | the exact causal insertion history over the retained work order | — | **yes (predecessor / insertion order is semantic proof structure)** | no (one `TraceCons` per work item — `outcome_trace_unique_step`) | structural projection (`trace_retained_cause`) | intrinsic inductive (follows the source-ordered `ewf_items`) | retained | the retained causal CONSTRUCTION history (exact tail trace + tail accumulator + current member + `StepCause`); a proof structure, NOT a general-purpose collection implementation |
| `GoCompile.ForestExprFactTable` (`feft_table`) / `ExprFactTable` (`prog_expr_facts`) | `NodeKey` -> `ExprFact` for `EOOk` outcomes | `NodeKey` (`mkKey FilePath positive`) | canonical (path then local id) | no (one entry per occurrence) | by NodeKey | **`FMapAVL`** (the SAME `NodeKeyMapBase` wrapper — no new collection) | retained | the SEALED occurrence-keyed expression-fact table — the `EOOk` PROJECTION of the SAME `ForestOutcomeTable` (`feft_is_facts`), not an independently computed authority; validated occurrence NodeKeys (no foreign/forged key — `prog_expr_facts_domain`); sealed into `ElaborationFacts` by object identity (`elaborate_ok_seals_facts`) |
| `GoCompile.TypeNameFactTable` (`build_type_name_fact_table` / `prog_type_name_facts`, §8) | type-name occurrence -> `TypeNameFact` (the resolved `GoType` ONLY) | `NodeKey` (`mkKey FilePath positive`) | canonical (path then local id) | no (one entry per type-name occurrence) | by NodeKey | **`FMapAVL`** (the SAME `NodeKeyMapBase` wrapper — no new collection) | retained | the SEPARATE SEALED occurrence-keyed type-name fact table (NOT part of any combined outcome root; each conversion's source name resolved by `predeclared_type` to a `GoType`; the fact stores the resolved type only, source identity stays in the `TypeNameRef`); domain = exactly the visited type-name occurrences (`prog_type_name_facts_domain` — no expression/foreign key); the total `type_name_fact_at_table` query PROJECTS the sealed table; retained in `ElaborationFacts` / `CompilableProgram` |
...
**Symbol-existence audit (current head).** Every code token named as current in this inventory was confirmed to
exist in the current theory, and its stated storage role / backing / order-and-duplicate claim was checked against
the current record or constructor: the C4 outcome/fact objects are `ExprWorkForest`/`AnnotatedExprWorkForest`
(ordered retained `list` views), `OutcomeAccumulator.oa_map` / `ForestOutcomeTable.fot_acc` (the `NodeKeyMapBase`
FMapAVL identity map — ONE entry per key, no list bucket, no overwrite fallback), `OutcomeTrace` (the intrinsic
causal inductive), `ForestExprFactTable`/`ExprFactTable` (the `EOOk` projection), and the separate
`TypeNameFactTable`; the current top-level function form is `DMain : list GoStmt -> GoDecl`. Intentionally absent
names are labeled as such and NOT listed as current storage: the deleted `prog_conv_outcomes` combined-outcome root,
the `TFun` constructor, the old `FMap.v` association list, and the old radix-trie `NodeTable` do not exist at the
current head and appear here only as rejected/historical. No deleted production authority is listed as current, no
custom general-purpose collection has returned, and no `ExprOutcome` "bucket value prevents overwrite" behavior
exists — outcome key uniqueness and the exact `oa_domain` prevent overwrite.
