...
| `GoCompile.analyze` folds / `GoTypes.occ_arg_typedb` over `occs_file`/`visit_file` | per-occurrence typing bits + fact/diagnostic/bucket accumulation | — | source preorder | n/a | `forallb` / `fold_right` / `flat_map` over the retained visit stream | `list` | retained | GoCompile's ONE indexed whole-program analysis folds the derived traversal enumeration (delegating to the SAME `expr_typedb`/`const_info` resolver); `GoTypes.occ_arg_typedb` is the leaf predicate, `occs_file_typedb_eq` its `= source_file_typedb` bridge; a proof/computation view over canonical enumerations, not storage (the C2 `indexed_program_typedb` peer checker is removed, §19/§26) |
...
| `GoCompile.collect_diagnostics` / `erased_report` (C3 §16/§17) | ordered diagnostic list / erased report | — | canonical (expression occurrences in visit order ++ package diagnostics in PackageMap key order) | no (n-1 duplicate-main entries retained) | positional / `map` | `list` | retained | the DERIVED canonical diagnostic enumeration (`flat_map`/`++` over the visit stream + bucket elements); `erased_report = map erase_diagnostic` projects it to a snapshot-independent `list ErasedDiagnostic` for cross-snapshot comparison; a report enumeration, not identity storage |
