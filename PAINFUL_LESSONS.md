...
10. **Handwritten OCaml is a filesystem exhaust pipe.** It receives only final relative paths and exact
    contents — it does not receive, inspect, validate, lower, render, or understand programs; it walks no
    Rocq terms; it chooses no paths or contents. All structural decoding (Coq `string`/`list`/`prod` → OCaml)
    is GENERATED by standard extraction, not handwritten. The deleted 82-line `Fido Emit` plugin — which
    accepted an arbitrary `constr`, decoded lists/records/strings by application ARITY, and could be handed a
    hand-built image bypassing `SafeProgram` — was term inspection at a lower layer, not a transport boundary.
    If the tiny-writer boundary cannot be met, delete the writer and the e2e; a false transport is worse than
    no integration.
...
18. **A copied `Compiled*` tree is not a "decoration" — it is a second AST.** Calling
    ...
    the ONE AST (`CompiledProgram := { ast : GoFile & GoCompile ast }`), or, when payloads must truly change,
    one phase-indexed AST family — never a parallel syntax hierarchy.
