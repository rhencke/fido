...
# extraction.  The ONE permitted handwritten OCaml is a filesystem exhaust pipe (`e2e/writer.ml`): a
# generic dirty-directory synchronizer that installs the already-computed image, decoding no program.
# This gate is literal and fail-closed:
#   (1) the set of tracked *.ml / *.mli / *.mlg is AT MOST { e2e/writer.ml };
#   (2) that writer does only file I/O — it must not walk/decode Rocq terms (no EConstr /
#       Constr / Nametab / interp / Typing / Reductionops / VERNAC / DECLARE PLUGIN), and must stay
#       bounded (a filesystem sync, not a program backend);
...
if [ -n "$extra_ocaml" ]; then
  echo "fido: OCAML-ORIGIN GATE — only the one filesystem sink ($glue) may be tracked OCaml.  Offending files:"
...
if git ls-files --error-unmatch "$glue" >/dev/null 2>&1; then
  ...
  if [ "$lines" -gt 200 ]; then
    echo "fido: OCAML-ORIGIN GATE — the writer glue is $lines lines; a filesystem sync must stay bounded (<=200)."
    exit 1
  fi
...
if [ -n "$hits" ]; then
  ...
  echo "fido: ONLY through the proved chain + standard extraction + the one dirty-directory filesystem sink."
  exit 1
fi

echo "fido: ocaml-origin gate OK — at most the one filesystem sink; no term-walking; no backend hallmarks ✓"
