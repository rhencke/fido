...
if [ "${FIDO_SELFTEST_NESTED:-0}" = 1 ]; then
  ...
  cp "$here/tools/ocaml-origin-gate.sh" "$here/tools/generated-output-gate.sh" \
     "$here/tools/generated-mode-gate.sh" "$here/tools/staged-generated-compare.sh" \
     "$here/tools/precommit-selftest.sh" "$repo/tools/"
  ...
  # SYMLINK-MODE bypass: main.go staged as a mode-120000 (symlink) entry whose blob equals the canonical
  # bytes.  Under core.symlinks=false the export materializes it as a plain regular file with the right
  # header/bytes, so the exported-object -L/-f/-x checks and the byte compare all pass — but the proposed
  # commit records mode 120000.  The index-authoritative generated-mode-gate (git ls-files -s) must reject it.
  ( cd "$repo"
    git reset -q --hard HEAD; rm -f .githooks/pre-commit-mut .githooks/pre-commit-sw .githooks/pre-commit-mode
    git config core.symlinks false
    blob=$(git hash-object -w main.go)                       # main.go holds the canonical generated bytes
    git update-index --cacheinfo 120000,"$blob",main.go ) >/dev/null 2>&1   # index: main.go as symlink mode, bytes unchanged
  out=$(run_hook "$repo/.githooks/pre-commit"); rc=$?
  if [ "$rc" -ne 0 ] && [ "$rc" -ne 7 ] && printf '%s\n' "$out" | grep -q 'GENERATED-MODE GATE'; then
    ok "the REAL hook rejects a mode-120000 (symlink) main.go with canonical bytes (index-mode gate, not the exported-object -L check)"
  else
    bad "the real hook did not reject a mode-120000 generated file (rc=$rc) — index-mode gate missing?"
  fi
  # mutation: a hook WITHOUT the index-mode gate lets the mode-120000 entry bypass (export → plain file → passes).
  grep -v 'generated-mode-gate' "$repo/.githooks/pre-commit" > "$repo/.githooks/pre-commit-mode"
  out=$(run_hook "$repo/.githooks/pre-commit-mode"); rc=$?
  if [ "$rc" -eq 7 ] && ! printf '%s\n' "$out" | grep -q 'GENERATED-MODE GATE'; then
    ok "dropping the index-mode gate lets the mode-120000 generated file bypass to Buildx — proving the gate is load-bearing (teeth)"
  else
