(** cmd_unified.v — the FIRST bridge between Fido's two proof-only semantics universes.
    ...
    value (grounded on [run_cmd_panic_char]), with [run_cmd] completion discharged internally by
    [run_cmd_terminates].  Plus cmd.v-side properties for ANY [c]: [run_cmd] TERMINATES for enough fuel, and — for
    a COMPLETING [run_cmd] — its output only APPENDS (never retracts) and a panic-free such run returns [ORet].
...
(** [run_cmd] TERMINATES for enough fuel, for ANY [c] (nested defers included): the body ([go]) plus the defer
    forest ([run_defers_terminates]).  A cmd.v-side property CONSUMED by [bridge_agrees] below to discharge its
...
Local Lemma unwind_prefix_panic : forall fuel ds acc result q0 val,
  ...
  forall ucap p b h lv tr o df pa ds_tail,
    ...
    exists (p' : nat -> UCmd) (df' : nat -> list UCmd) (pa' : nat -> option GoAny) evs,
      usteps ucap (mkUCfg p b h lv tr o df pa)
...
Proof.
  ...
  destruct (unwind_prefix_panic fuel (cmd_defers c)
              ...
              ucap p' (fun _ => nil) (fun _ => 0) (fun t => Nat.eqb t 0) nil
  ...
  assert (Hfin : exists (lvF : nat -> bool) (paF : nat -> option GoAny),
             usteps ucap (mkUCfg p'' (fun _ => nil) (fun _ => 0) (fun t => Nat.eqb t 0) nil
                            ((nil ++ map (fun e => (0, e)) (cmd_out_events c)) ++ map (fun e => (0, e)) evs) df'' pa'')
                         (mkUCfg p'' (fun _ => nil) (fun _ => 0) lvF nil
