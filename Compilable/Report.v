...
Definition Issue (r : AN.Result p) : Type := AN.Issue (AN.res_surface r).
Definition IssueRef (r : AN.Result p) : Type := AN.IssueRef r.
Definition result_issues (r : AN.Result p) : list (AN.Issue (AN.res_surface r)) := AN.result_issues r.
Definition ir_ord {r : AN.Result p} (ref : AN.IssueRef r) : nat := AN.ir_ord ref.
Definition ir_row {r : AN.Result p} (ref : AN.IssueRef r) : AN.Issue (AN.res_surface r) := AN.ir_row ref.

(* the exact class, root, family, cause-or-requirement, and per-class row an issue projects *)
Definition issue_class {r : AN.Result p} (i : AN.Issue (AN.res_surface r)) : AN.IssueClass := AN.issue_class i.
Definition issue_root {r : AN.Result p} (i : AN.Issue (AN.res_surface r)) : AN.IssueRoot (AN.res_surface r) := AN.issue_root i.
...
Lemma issue_membership_complete (r : AN.Result p) (n : nat) (i : AN.Issue (AN.res_surface r)) :
...
Lemma issue_stable_diag (r : AN.Result p) (n : nat) (d : AN.Diagnostic (AN.res_surface r)) :
...
Lemma issue_stable_bound (r : AN.Result p) (n : nat) (b : AN.Boundary (AN.res_surface r)) :
