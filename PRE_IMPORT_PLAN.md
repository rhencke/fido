...
Keystones:
- [~] **A — full-width int model** — A1 (u32/i32_mul) ✓; A2 signed `GoI64` ✓; A3 `GoU64` ✓; A4 default-`int` migration ✓ (A4.1 concurrency-bridge carrier → `GoI64`; A4.2a ergonomic range-checked `%i64`/`%u64` literals + scoped arith; A4.2b Comparable + int64/uint64 chan+map pipelines + `int` reframing; A4.3a overflow-safe arithmetic on full-width `GoI64` [`i64_add_nz`/`sub_nz`/`mul_nz` + exact theorems], `overflow_safe_demo` migrated — `GoI64`/`GoU64` are THE canonical int64/uint64 and Fido's provable-overflow-freedom now operates at the true int64 width); A5 untyped-constant *arithmetic* pending (literals already range-checked at parse).  *Demo-source note:* the remaining `int` uses are FEATURE demos where an int value is incidental (channel/map/session payloads — already proven to carry `GoI64` by the pipelines; struct/method/interface fields; control-flow loop counters), slice/string INDICES (correctly Go `int`), and two plugin-feature demos (`prec_demo` precedence printer, `neglit_demo` Sint63-signed-literal printing) — not "the int model", so left as `Sint63 int` (golden-identical to convert; would only refocus each demo).
