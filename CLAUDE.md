...
consumed minimally by `GoTypes` (`indexed_program_typedb` let-binds ONE `index_program`, taking each role from
the precomputed index via the reference and each syntax fragment from the delivered occurrence — no rebuild, no
source recovery — proved `= program_typedb`, bytes UNCHANGED). · `GoTypes` — the ONE type
...
- **Certified theory** (`dune`): `digits.v`, `Ints.v`, `Floats.v`, `Complexes.v`, `FilePath.v`,
  ...
  `indexed_program_typedb` let-binds ONE `index_program p` and folds `visit_file`'s `(NodeRef, syntax)` pairs,
  taking each occurrence's ROLE from the index THROUGH the reference (`node_role idx (fst rocc)` — an
  outer-FileMap + inner-PositiveMap lookup in the PRECOMPUTED `si_outer`, no rebuild) and its SYNTAX from the
  DELIVERED fragment (`view_expr (snd rocc)` — no `node_at`/`source_occurrence_of_ref` recovery), delegating to
  the SAME `expr_typedb`/`const_info` resolver, and is proved EQUAL to `program_typedb` (no semantic kernel
  modified — the indexed checker is an added surface, not a rewire).
