...
consumed minimally by `GoTypes` (`indexed_program_typedb` folds the delivered syntax fragments — no source
recovery — proved `= program_typedb`, bytes UNCHANGED). · `GoTypes` — the ONE type
...
- **Certified theory** (`dune`): `digits.v`, `Ints.v`, `Floats.v`, `Complexes.v`, `FilePath.v`,
  ...
  EQUAL to `program_typedb` (no semantic kernel modified — the indexed checker is an added surface, not a rewire).
