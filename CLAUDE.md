...
- **Certified theory** (`dune`): `digits.v`, `Ints.v`, `Floats.v`, `Complexes.v`, `FilePath.v`,
  ...
  through the sealed API).  The isolated C0 spike `OccurrenceSpike.v` it generalized is DELETED (no parallel
  spike authority remains).  Current Go behavior and every generated byte are UNCHANGED by `GoIndex`.
