...
consumed minimally by `GoTypes` (`indexed_program_typedb` = `program_typedb`, one let-bound index, bytes
UNCHANGED). · `GoTypes` — the ONE type
...
- **Certified theory** (`dune`): `digits.v`, `Ints.v`, `Floats.v`, `Complexes.v`, `FilePath.v`,
  ...
  `indexed_program_typedb` let-binds ONE `index_program`, threads it through `visit_file`, takes each decision
  through the total query API delegating to the SAME `expr_typedb`/`const_info` resolver, and is proved EQUAL
  to `program_typedb` (no semantic kernel modified — the indexed checker is an added surface, not a rewire).
