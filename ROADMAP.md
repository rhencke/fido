...
|---|---|---|---|---|---|
| ASSUM-GATE | DOC-RESET | Settle what the readable assumption surface in `gate/Assumptions.v` is for, and make its printed claim equal what it verifies. | A readable gate whose declared set has a stated rule, or no readable gate. | — | resolves `ASSUMPTION-GATE-PLACEMENT`; `make prove` green; zero project axioms unchanged |
