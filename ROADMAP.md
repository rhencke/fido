...
|---|---|---|---|---|---|
| ASSUM-GATE | DOC-RESET | Delete the readable assumption surface `gate/Assumptions.v`, leaving one enforcement chain to own the build-time zero-project-axiom claim. | One chain — certified-module coverage, then the whole-theory audit, then adversarial controls A-E — with no part sufficient alone and no hand-maintained list beside it. | — | `ASSUMPTION-GATE-PLACEMENT` accepted; `make prove`, `make check`, `make audit-fresh` green; controls A-E still execute; generated bytes and goldens unchanged |
