...
Definition r_neg_uint8   : Compilable.rejects (prog [ PL [ NEG (CONV Names.Uint8 (ILIT 1)) ] ]).        Proof. reject. Qed.
Definition r_neg_int8min : Compilable.rejects (prog [ PL [ NEG (CONV Names.Int8 (NEG (ILIT 128))) ] ]). Proof. reject. Qed.
...
Definition r_cx_int8 : Compilable.rejects (prog [ PL [ CPLX (CONV Names.Int8 (ILIT 1)) (CONV Names.Int8 (ILIT 2)) ] ]). Proof. reject. Qed.
Definition r_cx_mix  : Compilable.rejects (prog [ PL [ CPLX (CONV Names.Float32 (ILIT 1)) (CONV Names.Float64 (ILIT 2)) ] ]). Proof. reject. Qed.

(* wrong arity: conversions take exactly one argument, the complex builtin exactly two *)
Definition r_conv0 : Compilable.rejects (prog [ PL [ APP (Names.type_name_ordinary Names.Int8) [] ] ]).              Proof. reject. Qed.
Definition r_conv2 : Compilable.rejects (prog [ PL [ APP (Names.type_name_ordinary Names.Int8) [ILIT 1; ILIT 2] ] ]). Proof. reject. Qed.
...
Definition r_type_value  : Compilable.rejects (prog [ PL [ Syntax.Name (Names.type_name_ordinary Names.Int8) ] ]).   Proof. reject. Qed.
...
Definition r_stmt_conv : Compilable.rejects (prog [ Syntax.ExprStmt (CONV Names.Int8 (ILIT 1)) ]). Proof. reject. Qed.
...
Definition c_neg_int8    : Compilable.compiles (prog [ PL [ NEG (CONV Names.Int8 (ILIT 1)) ] ]). Proof. compileok. Qed.
