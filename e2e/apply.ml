(* apply — the tiny INTERNAL publication adapter for the `make regenerate` / `sync` workflow.  It reads the
   fixed pristine source [/generated] (the certified generated-module layer — materialized by `Fido Materialize`
   and validated by the go-e2e Docker stage) and synchronizes it into the destination through [Sink.sync].
