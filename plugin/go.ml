...
let go_prim_type_table = [
  ...
  (* NOTE: the fixed-width [int8]…[uint64] are NOT here — they are the distinct records
     ...
     bare-[int] alias entry here would be cross-assignable in Rocq yet render a distinct Go type
     ⇒ invalid Go — never re-add one. *)
]
...
(* Struct pointers (Phase Bs.2): GSPtr R → Go *R; ops lower to native pointer syntax, reusing the
   same shapes as Ptr.  The StructRep/field-cell machinery is proof-only (the lowering never touches
...
let is_gsptr_machinery r =
  ...
  List.mem (global_basename r)
    ["gsptr_new"; "gsptr_deref"; "gsptr_assign"; "gsptr_get_field"; "gsptr_set_field";
     ...
     (* Bs.1 field-cell substrate (proof-only; dragged in by the gsptr op bodies) *)
...
let go_type_tag_map = [
  "TBool",    "bool";
  "TInt64",   "int";      (* PrimInt63 = Go's platform int (TInt64 is the sole int tag) *)
  "TFloat64", "float64";
  "TString",  "string";
  "TUint",    "uint";
  (* canonical Squash-sealed fixed-width family — one tag per Go type (no bare-int duplicates). *)
  "TU8",   "uint8";  "TI8",  "int8";
...
(* [f64_of_int i] / [f64_of_i64 a] / [f64_of_f32 a] / [f64_of_u64 a] — int (Sint63) / int64 (GoI64) /
   float32 / uint64 → float64 (the recognizer's full domain): Go's native
   [float64(x)] (the carrier converts directly to the nearest double; Go does the
...
let is_int_to_f32_ref = named_in ["f32_of_int"; "f32_of_i64"; "f32_of_u64"]
...
(* The Go operator SURFACE and PRECEDENCE are owned by the VERIFIED [GoPrint] definitions
   ...
   — one operator authority, no parallel precedence/surface tables. *)
...
let rec pp_type state = function
  ...
  (* Each numeric wrapper renders as its REAL Go type — GoU8→uint8 … GoI32→int32,
     GoI64→int64, GoU64→uint64 (a value's static / interface type is faithful;
  ...
  | Tglob (r, []) when is_uint63_type r -> str "int"   (* PrimInt63 = Go's platform [int]
       (DISTINCT from the Z-carried [GoI64]=int64, so a boxed int ≠ a boxed int64
       and [tag_eq TInt64 TI64 = None] AGREES with Go's [v.(int)] vs [v.(int64)]). *)
...
(* A CONVERSION's type must be PARENTHESISED when it starts with [func], [*] or [<-] (Go spec,
   "Conversions": otherwise [func(A) B(x)] / [*T(x)] / [<-chan T(x)] are AMBIGUOUS).  The rule is applied
   to the RENDERED type's leading token — not the ml_type constructor — so it also covers a defined type
   whose underlying is the [GoFunc] tag ([type Handler func(A) B], a [Tglob]); the plugin emits CORRECT
   Go itself, never leaning on gofmt's silent repair. *)
...
(*s Check whether a given de Bruijn index appears free in an expression.
    ...
    continuation [fun v ok => …]) is not mistaken for the outer index — an unshifted
    descent would false-negative and defeat discard-detection on left-nested [>>']
    chains.  [db1_free] = "the bind result (index 1) is unused". *)
...
(* ---- The verified [GoPrint] expression printer, wired LIVE ----
   ...
   human-facing summary lives ONCE in PROGRESS.md (do not re-enumerate it here).
...
let rec pp_expr state env = function
  ...
  | MLapp (head, args) ->
      ...
      (match head, vis with
       ...
       (* send tag ch v → ch <- v.  A narrow channel element ([chan uint8] <- int64-carried value)
          needs the [uint8(…)] cast; the tag is the element type.  Non-narrow sends keep
          the bare [pp_expr]. *)
       ...
       (* map_make_typed kt vt → make(map[K]V) with concrete types from tags.  AT THIS ARM the tag already
          ...
          NOT the global map-key authority.  Type rendering itself goes through [go_type_of_tag], which carries
          its OWN [goty_comparable_key] check, so a non-comparable map key reaching ANY emitted type path (e.g.
          [make_chan]) fails loud there ([negtests/neg_chan_bad_map_key]) — [MapKeysOk] (model) and
          [goty_comparable_key] (plugin) are now DUPLICATE map-key checks the general certified type authority
          ([GoTypeDesc]) should UNIFY. *)
       ...
       (* str_len s → len(s): Go's [len] already gives the BYTE count as a Go [int],
          which is exactly the [int] (PrimInt63) model — no cast. *)
       ...
       (* platform-uint smart constructor [uint_lit z _] → [uint(<unsigned decimal>)] (Go's typed
          uint conversion).  EXACTLY [u64_lit]'s shape: [GoUint] is [Z]-carried and
       ...
       (* [f64_of_int]/[f64_of_i64]/[f64_of_f32]/[f64_of_u64] — int/int64/float32/uint64 → float64: Go's
          native cast; likewise bridged to [ECall (EId "float64") [x]] in the binop-operand case. *)
       ...
       (* [f32_of_int]/[f32_of_i64]/[f32_of_u64] — DIRECT int → float32: Go's [float32(x)] cast
          (rounds the integer once, unlike the double-rounding [float32(float64(x))]); likewise bridged to
       ...
       (* [u64_of_f64 f] → [uint64(f)] (float64 → uint64 truncation toward zero); likewise bridged to
       ...
       (* narrow → int64 widening → [int64(x)] (Go's widening conversion).  NOT identity: the narrow-boundary
          ...
          is already an int64-carried expression — so it is correct in EVERY position.  (When this cast is a
          binop operand and [x] bridges, [goexpr_bridge] now builds it as [ECall (EId "int64") [x]] and the
          verified [gprint] emits these exact bytes; this arm still serves every other position.) *)
       | MLglob r, [x] when is_i64_of_narrow_ref r ->
           str "int64(" ++ pp_expr state env x ++ str ")"
       (* float64 → float32 narrowing (round-nearest-even) → [float32(x)].  A CONSTANT that
          overflows float32 is a Go compile error, so force runtime (the float64 operand fits).  (The
          [operand_is_runtime] inline [float32(x)] case, as a binop operand, is now bridged to [ECall
          (EId "float32") [x]] via [goexpr_bridge] — same guard — and printed by [gprint]; the force-wrapper
          IIFE branch is NOT bridged and stays here.) *)
  ...
  | MLletin (_id, e1, e2) ->
      (* VALUE-POSITION-LET RULE: a value-position let must NOT become `(func() any { x := e1; return e2 })()` — the
         `any` return is a non-compiling type in a typed context (e.g. `int64(any)` inside int64
         arithmetic).  An expression-position let is PURE (referentially transparent), so
      ...
      (match nat_value e with
       ...
       | None   ->
           ...
           (* [any v] (existT) in value position → its payload [v], which Go boxes to
              ...
              narrow payload is converted to its real Go type at this boundary. *)
           ...
           | MLcons (_, r, [s; m; e]) when String.equal (global_basename r) "S754_finite" ->
               ...
               (* CANONICITY GATE (fail-closed): GoFloat64's carrier invariant is binary64-
                  canonical (SFeqb/SFcompare are representation-sensitive).  The check is the
                  VERIFIED SpecFloat.bounded (canonical mantissa + exponent bound), extracted
                  as Printer.f64_bounded and applied to the STRUCTURALLY converted fields —
                  one authority, no hand-rolled approximation. *)
               (match sign_opt, coq_pos_of_ml m, coq_z_of_ml e with
                | Some _, Some mp, Some ez
                  when (match Printer.f64_bounded mp ez with
                        | Printer.True -> false | Printer.False -> true) ->
                    unsupported "a NONCANONICAL spec_float literal (SpecFloat.bounded rejects it) — GoFloat64 carries only canonical binary64 values"
           ...
           | MLcons (_, r, _) as lst ->
               (* A non-empty list literal in VALUE position.  `append(nil, v1, …)`
...
(* Print [e] as an operand inside a binary operator whose context requires precedence [>= ctx].
   ...
   [pp_expr]).  The gprint-representable sub-class (the live bridge — see [goexpr_bridge]) delegates
            ...
            | [ (_, p1, body1); (_, p2, body2) ] ->
                ...
                (match cref p1, cref p2 with
                 ...
                 (* MATCH-SHAPE guard: a 2-branch match that is NOT bool true/false would silently become a
                    bare `return`, DROPPING both arms — fail loud. *)
...
let proof_only_names =
  [ "ref_sel"; "ref_sel_opt"; "ref_upd"; "ref_install";
    ...
    "slice_range_live";  (* source-liveness guard inside slice_append's realloc — proof-only *)
