...
- **Clean tractable wins ON DECK (review 2026-06-28):** ✅ Stage B slice 1 DONE — the first `Front` expression class (`MLrel OP MLrel`) is live in the plugin (see the PRINTER ledger). REMAINING: widen the live Front bridge (literal / nested-binop operands, then atoms / calls) while collapsing the duplicate operator authority (`front_binop_of` vs `binop_of`) toward a single op source; make `gofmt` check-only (`gofmt -l` → fail if it would rewrite) to pull it out of the trusted path; collapse the duplicate type parsers (prove the type round-trip through `Front.parse_gty`, then retire the string-level `parse_ty`). These are real, bounded wins — not "frontier."
