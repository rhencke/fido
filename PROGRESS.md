...
- **C2 — production occurrence index, snapshot-local references, and indexed traversal (LANDED; pending the
  ...
  MINIMALLY: `indexed_program_typedb` let-binds ONE `index_program`, threads it through `visit_file`,
  delegates each decision to the SAME `expr_typedb`/`const_info` resolver via the total query API, and is
  proved EQUAL to `program_typedb` — no semantic kernel modified, every generated byte unchanged.
