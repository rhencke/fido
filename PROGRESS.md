...
- **C2 — production occurrence index, snapshot-local references, and indexed traversal (LANDED; pending the
  ...
  GoTypes owns only the type/constant relation + the per-occurrence predicate `occ_arg_typedb`; the C2
  `indexed_program_typedb` peer whole-program checker is removed (§19/§26).  Every generated byte unchanged.
