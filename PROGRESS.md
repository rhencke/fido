...
- **C1 — specification-shaped file roots (DONE; storage corrected by C1A/C1B):** replaced the project-authored
  ...
  standard maps, and installed the binding collection law.  Later checkpoints (C2 production `GoIndex` over the
  standard maps; C3 occurrence-anchored diagnostics; C4 source type-name syntax; C5 remaining predeclared
  numerics) each need explicit sign-off before starting.  (Proof-only source recovery is NOT a hot-path
  guarantee — ordinary compilation receives the syntax fragment and its `NodeRef` together.)
