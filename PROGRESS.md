...
over runtime locals, int/int64/uint64 literals, and the bare int64/uint64 complement `^x`; every other shape is
...
- **Main output is the legacy path.** `main.go` is produced by the trusted plugin, not the certificate-gated
  ...
  (locals / int·int64·uint64 literals / int64/uint64 complement `^x`); everything else is trusted `pp_expr`.
