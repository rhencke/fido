...
- **PRINTER (status ledger).** The `SRaw` verified-expression-printer overlay was deleted (postmortem in `LESSONS.md`; do not revive it). Current state:
  ...
  - **NEXT** — ✅ operator authority UNIFIED (the hardcoded `go_infix`/`op_prec`/`float_prec` tables in `go.ml` are deleted; `binop_of` now returns the verified Front `binOp` and surface+precedence are derived from Front's `binop_text`/`binop_prec` — one operator authority; golden byte-identical). REMAINING: grow the LIVE Front class — bridge binops with literal / nested-binop operands (subsuming `pp_prec`'s force-wrapper decision so the trusted printer shrinks), then atoms / calls / conversions. Each step: construct `Front.GExpr` directly, never via string parsing; keep golden byte-identical (or bless an intended delta).
