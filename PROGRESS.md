...
- **PRINTER (status ledger).** The `SRaw` verified-expression-printer overlay was deleted (postmortem in `LESSONS.md`; do not revive it). Current state:
  ...
  - **NEXT** — ✅ operator authority UNIFIED (`binop_of` returns the verified Front `binOp`; surface+precedence from Front). ✅ the live bridge is RECURSIVE over binop TREES of runtime locals (`a*b+c`, `(a+b)*c`), per-node force-safety guard. ✅ platform-int LITERAL operands bridged (`a + 5`): `int_lit` prints bare via `Printer.print_Z` = the plugin's `print_i64_dec`, so `Front.EInt` matches by construction (liveness shown by a +100 perturbation flipping every bridged int literal). REMAINING: `i64_lit`/`u64_lit` operands need a conversion node (`int64(N)`) — gated on Front gaining `EConv`; then unary (Front's `EUn` parenthesises its operand, so it is NOT byte-equal to the plugin's idiomatic `^x` — needs an idiomatic-print mode or a blessed delta) / atoms / calls. Each step: construct `Front.GExpr` directly, never via string parsing; keep golden byte-identical (or bless an intended delta).
