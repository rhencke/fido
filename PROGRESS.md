...
- **VERIFIED PRINTER (2026-06-25) — the print path moved INTO Rocq, with an UNCONDITIONAL ROUND-TRIP _for the Rocq expression grammar_ (printer/parser SELF-CONSISTENCY — NOT a "Go parses this text the same way" theorem; that recognition theorem is the open gap-#10 printer face below).** `goprint.v` defines the Go type/expression/literal printers; they are EXTRACTED to `plugin/printer.ml` and the plugin calls the *extracted* functions (no hand-written OCaml printing for these — it runs the same code Rocq reasons about). Every printer has an UNCONDITIONAL round-trip theorem (against the *Rocq* parser), ZERO axioms: `print_ty`/`print_ty_inj` (types incl. validated nominal `Ident`s — invalid names unrepresentable), `print_parse_expr` (expressions, precedence-CORRECT — `EAtom` carries a structured `GoAtom`, so a malformed atom is unrepresentable and the theorem needs NO side-condition), `print_parse_Z`/`_hex`/`_float_hex`, `esc_string_roundtrip`, `print_sep`. `build_goexpr`'s atom guard FAILS LOUD on any non-structured operand (the executed path enforces what the type demands). A Docker gate (non-bypassable *in CI*; see the smart-ctor gate caveat — it is a static-discipline gate, not a type-level seal) asserts zero axioms + `printer.ml`-in-sync on every build. **Remaining = gap #10's printer face:** the round-trips are printer/parser SELF-CONSISTENCY for the *Rocq* grammar, NOT a recognition theorem for Go's OWN parser; richer Go structure to retire the `atomic` string-lexer and shrink the opaque `SRaw` hatch — DONE: `AIdent`/`AIntLit`/`AStringLit`, a quote-aware scanner (string literals are opaque to the bracket/seam counter), the **full postfix `PrimaryExpr` grammar `SSelector`/`SIndex`/`SSlice`** (`x.f` / `a[i]` / `a[lo:hi]`, index/slice children are GoExprs printed at ctx 0, recovered by `scan_base`(operand)+`parse_postfix`(spine) — replacing opaque `SRaw` for those shapes), and `EUnary` (a verified prefix GoExpr node: `!`/`^`/`*`/`&` print bare, and **`UNeg` `-x` prints PARENTHESISED `-(e)` and dispatches on the unambiguous `-(` prefix** — a bare `-x` would collide with the `-5` literal — closing the latent `build_atom` abort where a float/`f32`/complex/`i64`/`u64` negation used as a binop OPERAND was a `raw_ok`-rejected `-x` atom; `build_goexpr` emits `EUnary` instead of a raw atom, `raw_ok` rejects unary-led strings); **Stage B `SApply` LANDED (review #9 A1+A3)** — a verified application node `operand ++ "(" ++ args ++ ")"` (mutual `ArgList` + the `parse_args` round-trip); `build_goexpr` now CONSTRUCTS it DIRECTLY (no `pp_expr→string→build_atom` rescue) for identifier/method CALLS (`f(x)`/`recv.M(a)`) AND identifier-led CONVERSIONS (`int64(x)`/`float64(x)` — token-indistinguishable from calls, amendment 3); `raw_ok` now REJECTS call-shaped strings. **`SHexLit` LANDED (A2)** — hex INT literals are a verified node (`raw_ok` rejects them; hex floats keep the raw hatch). The irreducible opaque `SRaw` core is now just **func-lit bodies / IIFE force-wrappers, RE-SCOPED to Phase-B `GoFuncLit`/`EFuncLit`** (A4 finding 2026-06-27, SHARPENED by the SFuncLitCall diligence): the pre-statement scan-split (`SFuncLitCall` — structure only the call ARGS, leave the func-lit body an opaque `raw_ok` string) is the amendment-5 **legitimate bounded hatch**, deliberately NOT pursued. A round-trip for it IS reachable (carry a bare-func-lit witness `scan_to_brace (len s) s = Some (s, "")` in the body sig, so `leaf_whole_base` recovers `body`+`(args)` via `scan_to_brace_app`), so the earlier "non-compositional / blocked" claim was too strong — but it is **deliberately abandoned** for two reasons: (1) it is only a half-measure (the body stays an opaque string, only the args get structured), and (2) its hardest obligation is a net-new ~100-line `scan_bal`↔`bstack` bisimulation (no existing bridge — `scan_bal` lives only inside `scan_func_base`), which is **throwaway** under the more-correct end-state. That end-state is a real `GoFuncLit` whose body is a `GoStmt` plus an `EFuncLit` GoExpr node parsed by the func-lit's OWN grammar (`parse_print_funclit`, LANDED in the B0 foundation slices 1-4) — a STRUCTURAL parse that never touches `scan_func_base`/`scan_bal`. So the func-lit-call SRaw correctly stays the Phase-B holdout (faithful: the printer emits byte-exact Go for it via the verified `SRaw` atom — opaque-but-correct, not plausible-but-wrong), and the next verified-printer milestone is **Phase-B `EFuncLit`** built on the landed statement/param/func-lit round-trips. And the gofmt'd FINAL bytes — the round-trips prove properties of `print_expr`'s OUTPUT (the *Rocq printer's* bytes), but `make extract` runs `gofmt -w`, so the COMMITTED bytes are `gofmt(plugin output)` via an explicitly-TRUSTED gofmt postprocessor (**review #4 item-4 DECISION: gofmt is trusted, not temporary** — see END-TO-END TRUST BASE; the honest claim is "verified-printer output, then trusted-gofmt-formatted," and bringing those bytes in-proof is the future close).
...
- **Remaining frontier — all deep / foundational / gated / unfixable** (no clean tractable win left): native `interface{}` keyword (idiomatic paradigm change; dict lowering already correct), defined-type map KEYS (heterogeneous-heap rework, niche), the verified-printer's Go-RECOGNITION theorem (its print FUNCTIONS are now proven with unconditional round-trips — see the VERIFIED PRINTER status above; what remains is proving the Rocq parser faithful to Go's OWN grammar + gofmt bytes in-proof — the gap #10 face), the concurrency IO-LIFT (spawn fragment; `go_spawn` has no `run_io` law — user-gated option b), CI (gh token lacks Actions perms), and `int` bitwise/shifts (63-bit Rocq-`int` substrate limit) / assert-to-`any` (`GoTypeTag GoAny` universe-inconsistent) — both genuine `✗` that cannot be closed under the current substrate.
...
- **The `gofmt` postprocessor** (Go's formatter, EXTERNAL) — `make extract` runs `gofmt -w` on the
  ...
  bytes.  The verified printer's round-trip theorems (`print_parse_expr` etc.) prove properties of
  `print_expr`'s OUTPUT — the *Rocq printer's* bytes — so they do NOT reach the gofmt'd committed
  bytes: gofmt is an explicitly-TRUSTED (unverified) rewrite.  **DECISION (external review #4 item 4):
  gofmt is TRUSTED, not "temporary-until-proven."**  Making `print_*` emit gofmt-canonical bytes
  directly (so gofmt is a no-op and committed == proven) is the future close, but gofmt is Go's ONLY
  definition of canonical surface form (no spec to implement independently), so replicating it
  byte-for-byte is the worse trade for now.  The honest claim is therefore **"verified-printer output,
  then trusted-gofmt-formatted,"** NOT "verified committed bytes."  (gofmt does not touch parens, so
  the *parenthesisation* — what `print_expr`'s precedence theorem actually certifies — survives gofmt
  unchanged; it rewrites only operator/operand whitespace.)
