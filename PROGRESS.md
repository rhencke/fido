...
- **PRINTER (status ledger).** The `SRaw` verified-expression-printer overlay was deleted (postmortem in `LESSONS.md`; do not revive it). Current state:
  ...
  - **NEXT** — ✅ operator authority UNIFIED (`binop_of` returns the verified Front `binOp`; surface+precedence from Front's `binop_text`/`binop_prec`; the hardcoded tables are gone). ✅ the live bridge is now RECURSIVE — a binop TREE whose leaves are all runtime locals (`a*b + c`, `(a+b)*c`) is printed in full by `Front.gprint`, with a per-node force-safety guard so it stays byte-equal to `pp_prec`. REMAINING: bridge LITERAL operands (`a + 1` — needs `print_Z` byte-matched to the plugin's int rendering), then atoms / calls / conversions, shrinking the `pp_prec` fallback. Each step: construct `Front.GExpr` directly, never via string parsing; keep golden byte-identical (or bless an intended delta).
