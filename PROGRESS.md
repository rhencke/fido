...
- **C0 — preflight residue + occurrence-index proof spike (done):** the three stale complex-era comments were
  repaired, and the complex-underflow scalar-conversion scar was added in kernel + pinned-Go differential form
  (`int(complex(3, 1e-50))` rejects on a nonzero imaginary; `int(complex64(complex(3, 1e-50)))` = 3 accepts
  because the explicit `complex64` boundary rounds the imaginary component to +0). `OccurrenceSpike.v` is an
  ISOLATED, TEMPORARY, axiom-free spike (NOT wired into the certified pipeline; nothing in it is imported by
  `GoAST`/`GoTypes`/…; deleted when the production `GoIndex` lands at C2) that validates snapshot-local
  occurrence identity on a toy `file → decls → stmts → nested-exprs` grammar: file-local `positive` preorder
  ids, `NodeKey`, a validated `NodeRef`, a SELECTED certified positive-key radix-trie `NodeTable` (over the
  rejected primitive-array / list candidates — see the ledger decision record), a one-pass builder, and the
  C0.4 structural theorem set (root id canonical / no parent, single non-root parent, parent↔child inverse,
  same-file parentage, containing-file recovery, once-only enumeration, decidable key identity, sound + complete
  preorder-interval ancestry, source-ordered children, deterministic construction, a no-structural-search
  builder, metadata-is-not-a-subtree-copy, and equal-leaves-distinct-refs). Generated bytes UNCHANGED. C1+
  (spec-shaped file roots, production `GoIndex`, occurrence-anchored diagnostics, source type-name syntax, the
  remaining predeclared numerics) each need explicit sign-off before starting.
