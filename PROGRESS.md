...
- RUNTIME tier: the GTInt-FRAGMENT arc R1–R7 is complete; NEXT = the TYPED-runtime tier (plans/typed-runtime-tier.md — per-width carrier OPERATIONS — `^int64(len ..)` et al. supported-but-undenoted, pinned three-wide by `typed_runtime_not_absent`); the GENERAL dyadic↔`SF*` theorem — each case shrinks the
