...
- **PRINTER (2026-06-28) — the `SRaw` verified-EXPRESSION-printer experiment was DELETED (see `LESSONS.md`).** It was a raw-string-rescue masquerading as verification: the `SAtom`/`GoExpr` AST carried an `SRaw` hatch (arbitrary `raw_ok`-validated text), so its round-trip theorems were *vacuous* for everything the hatch carried. Five iterations *narrowed* `SRaw` (`SApply`/`SHexLit`/`EUnary`/`SConvert`/`SForceCall`) instead of deleting it. When finally torn out wholesale it proved **pure removable overlay** — golden BYTE-IDENTICAL at every step — confirming the plugin's TRUSTED OCaml `pp_expr` string printer had been doing the real work all along (`goprint.v` 7144→1527 lines, `printer.ml` 4053→1475). What SURVIVES is the genuinely-clean part: the Go **type and literal** printers (`print_ty`/`print_Z`/`print_string_lit`/`print_hex`/`print_float_hex`/`print_sep`) — EXTRACTED to `plugin/printer.ml`, called by the plugin, each with a real ZERO-AXIOM round-trip (`print_ty_inj`, `parse_print_ty`, `print_parse_Z`/`_hex`/`_float_hex`, `esc_string_roundtrip`) — plus the from-scratch Wirth **`Module Front`**, whose round-trip is now machine-checked at **ZERO AXIOMS over the FULL core + every postfix form + the first TYPE-using form**: a `lex : string → list Token`, a recursive-descent precedence-climbing token `parse`, a clean `GExpr` AST (`EId`/`EInt`/`EUn`/`EBn` + the five postfix forms `ESel` selector / `EIndex` index / `ESlice` two-bound slice / `ECall` variadic call / `EAssert` type assertion `e.(T)` — **NO raw constructor**; by construction it cannot represent unstructured text), and the **end-to-end round-trip `parse_print_roundtrip : ∀ e, parse_str (gprint 0 e) = Some (e, [])`** — composed from M3b `gtokens_lex : lex (gprint ctx e) = Some (gtokens ctx e)` (a rest-threaded lexer-seam induction) and M3c `gtokens_parse` (the classic left-spine precedence-climbing decomposition `lspine`/`parse_climb_pairs`/`all_Pexpr`, plus a postfix-spine `pspine`/`parse_postfix_pairs` for sel/index/slice/call/assert) — plus the printer-injectivity corollary `gprint_inj`. The variadic `ECall` (a `list GExpr` child) needed a custom induction principle `GExpr_ind'` (the auto-generated one gives no IH for list elements) and a MAX-based arg-list parse fuel `af`. `EAssert` is the first form whose child is a **type**: its `GoTy` is round-tripped through the M5 token-level type layer (`gttokens_ty` mirrors `print_ty`'s surface, `parse_gty` is a recursive-descent type parser, and `parse_gty_roundtrip : tsize t ≤ F → parse_gty F (gttokens_ty t ++ rest) = Some (t, rest)` closes it — all zero-axiom), so the assertion's type child needs no `Pexpr`/size-IH threading. That machinery now makes the remaining forms tractable. HONEST SCOPE: this is printer/parser **self-consistency for the clean Rocq grammar**, NOT yet a theorem about Go's own parser; and the grammar is currently the **binop/unary/atom CORE + selector/index/slice/call/type-assertion** — still being grown form-by-form (conversion / composite-literal / func-lit — the token-level type parser is now in place) toward replacing the trusted `pp_expr` (**Stage B — not yet done**). **Until Stage B, Go EXPRESSIONS are still printed by the trusted OCaml `pp_expr`** — the same trust status as before the `SRaw` experiment, minus the false "verified printer" claim. The real gap (#10 — a source→Go recognition/correctness theorem against Go's OWN grammar) is unchanged, and is now to be built on `Front`, not `SRaw`.
