...
- GROW `eval_value`/`denote_expr` (`len` over RUNTIME elements/values + `int(x)` of runtime `x` need a
  runtime-value layer, B3; the GENERAL dyadic-fold↔`SF*` agreement class theorem) — each case shrinks the
