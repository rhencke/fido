...
- The dyadic↔`SF*` agreement arc (`plans/dyadic-sf-agreement.md`, the per-rung ledger):
  ...
  runtime-only); rung 7: ADD + SUB + MUL at
  binary32 CLOSED (`sf_render_{add,sub,mul}_agrees_f32`, the checker's own composite; the
  ADD + MUL stacks precision-generic). Next: DIV at binary32 + conversions, then rung 8. (T1–T5 + R8 COMPLETE — `LESSONS.md`.)
