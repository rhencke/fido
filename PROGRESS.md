...
- **VERIFIED PRINTER (2026-06-25) — the print path moved INTO Rocq, with UNCONDITIONAL correctness.** `goprint.v` defines the Go type/expression/literal printers; they are EXTRACTED to `plugin/printer.ml` and the plugin calls the *extracted* functions (no hand-written OCaml printing for these — it runs the same code Rocq reasons about). Every printer has an UNCONDITIONAL round-trip theorem, ZERO axioms: `print_ty`/`print_ty_inj` (types incl. validated nominal `Ident`s — invalid names unrepresentable), `print_parse_expr` (expressions, precedence-CORRECT — `EAtom` carries an `Atom = {s | atom_ok s = true}`, so a malformed atom is unrepresentable and the theorem needs NO side-condition), `print_parse_Z`/`_hex`/`_float_hex`, `esc_string_roundtrip`, `print_sep`. `build_goexpr`'s atom guard FAILS LOUD on any non-`atom_ok` operand (the executed path enforces what the type demands). A non-bypassable Docker gate asserts zero axioms + `printer.ml`-in-sync on every build. **Remaining = gap #10's printer face:** the round-trips are printer/parser SELF-CONSISTENCY for the *Rocq* grammar, NOT a recognition theorem for Go's OWN parser; richer Go-primary atom constructors (`AIdent`/`AIntLit` DONE; `ACall`/`AStringLit`/… next) to retire the `atomic` string-lexer; and the gofmt'd FINAL bytes — the round-trips prove properties of `print_expr`'s OUTPUT (the *Rocq printer's* bytes), but `make extract` runs `gofmt -w`, so the COMMITTED bytes are `gofmt(plugin output)` via an explicitly-TRUSTED gofmt postprocessor (**review #4 item-4 DECISION: gofmt is trusted, not temporary** — see END-TO-END TRUST BASE; the honest claim is "verified-printer output, then trusted-gofmt-formatted," and bringing those bytes in-proof is the future close).
