...
- The dyadic↔`SF*` agreement arc (`plans/dyadic-sf-agreement.md`, the per-rung ledger):
  ...
  runtime-only); rung 7: ALL FOUR op endpoints
  at binary32 CLOSED (`sf_render_{add,sub,mul,div}_agrees_f32`, the checker's own composite;
  the ADD/MUL/DIV cores precision-generic). Next: cross-width conversions, then rung 8. (T1–T5 + R8 COMPLETE — `LESSONS.md`.)
