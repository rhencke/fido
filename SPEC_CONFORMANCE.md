...
- **(R2) cmd.v `CDfr` / `run_defers` — the FAITHFUL model:** `cmd.v` models `defer` as `CDfr d k`;
  ...
  every defer's effects happen.  `bridge_agrees` proves the `ustep` run AGREES with this for every `no_heap` command; `bridge_heap_agrees` covers ANY completing command (heap ops and defers included, final heaps agreeing).
