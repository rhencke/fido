...
(* [prog_expr_facts] is the source-determined SPECIFICATION ([add_occ_fact] per node); the production fact map
   [forest_facts] is a TOTAL PROJECTION of the ONE retained work forest object's [ewf_items] over the proof-
   carrying [ForestOutcomeTable] (extract each work item's [EOOk] fact via [total_forest_outcome_at] — no per-
   occurrence [const_info] rescan), proved EQUAL to this specification by [forest_facts_eq_spec]. *)
...
(* an outcome that is NOT a success — used to state a conversion child-failure ([SCChildFail] carries
...
(* §7 the EXACT-DOMAIN invariant: every present key is the node key of a visited EXPRESSION occurrence (a Some
   ...
   FOREIGN (unvisited) key; the accumulator carries it as [oa_domain] and — with coverage ([oa_covers]) — ties
   the table's domain EXACTLY to the retained [ExprWork] enumeration.  The fold-step freshness it needs
...
(* splitting the retained forest object at a member induces the matching visit split (order correspondence),
   DERIVED from the stored fields ([ewf_items_exact] + [ewf_pairs_nodup]) — no [build_expr_work_forest] re-call. *)
...
Inductive StepCause {p} {input : CompilationInput p} (forest : ExprWorkForest input)
    ...
    (acc_rest : OutcomeAccumulator forest tnft rest) : ExprOutcome p -> Prop :=
| SCLeaf : forall ci,
...
Proof.
  unfold expr_ref_role. rewrite (ew_erase_exact w),
    (prog_visit_occ_is_source p (ew_node_ref w) (ew_occurrence w) (ci_in_prog (ew_in_visit w))). reflexivity.
Qed.
...
Lemma conv_step_matches {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (current : ExprWork input) (rest : list (ExprWork input))
    (acc_rest : OutcomeAccumulator forest tnft rest) ts x (step : ConversionStep forest current rest ts x)
    (He : ew_expr current = EConvert ts x)
    (Hops : forall sm_op : SuffixMember forest rest,
       outcome_matches (ci_idx input) (ew_node_ref (sm_work sm_op)) (ew_occurrence (sm_work sm_op))
         (oa_total acc_rest sm_op)) :
  outcome_matches (ci_idx input) (ew_node_ref current) (ew_occurrence current)
    (conv_outcome tnft (ew_expr_ref current) (cw_target_ref (cs_conversion step))
       (ew_expr_ref (proj1_sig (cw_operand_work (cs_conversion step))))
       (oa_total acc_rest (cs_operand_suffix step))).
Proof.
  pose proof (Hops (cs_operand_suffix step)) as Hopm.
  unfold sm_work in Hopm. rewrite (cs_operand_exact step) in Hopm.
  assert (Hoprv : GoIndex.view_expr (ew_occurrence (proj1_sig (cw_operand_work (cs_conversion step)))) = Some x)
    by (rewrite (ew_view_exact (proj1_sig (cw_operand_work (cs_conversion step)))),
          (cw_operand_expr (cs_conversion step)); reflexivity).
  assert (Htnf : tnf_type (type_name_fact_at_table tnft (cw_target_ref (cs_conversion step))) = predeclared_type ts)
    by (rewrite (type_name_fact_at_table_resolves tnft (cw_target_ref (cs_conversion step)) ts
                   (cw_target_syntax (cs_conversion step))); reflexivity).
  exact (conv_stored_matches (ci_idx input) tnft (ew_node_ref current) (ew_expr_ref current)
           (ew_expr_ref (proj1_sig (cw_operand_work (cs_conversion step)))) (cw_target_ref (cs_conversion step))
           (ew_occurrence current) (ew_occurrence (proj1_sig (cw_operand_work (cs_conversion step)))) ts x
           (ew_as_expr_exact current)
           (ltac:(rewrite (ew_view_exact current), He; reflexivity))
           (ew_occ_role current) Htnf
           (cw_target_ref_eq (cs_conversion step)) (cw_operand_ref_eq (cs_conversion step))
           (oa_total acc_rest (cs_operand_suffix step))
           (outcome_matches_proj (ci_idx input) (ew_node_ref (proj1_sig (cw_operand_work (cs_conversion step))))
              (ew_occurrence (proj1_sig (cw_operand_work (cs_conversion step))))
              (oa_total acc_rest (cs_operand_suffix step)) Hopm)
           Hoprv).
Qed.
...
Lemma stepcause_matches {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (current : ExprWork input) (rest : list (ExprWork input))
    (acc_rest : OutcomeAccumulator forest tnft rest) o :
  StepCause forest tnft current rest acc_rest o ->
  (forall sm_op : SuffixMember forest rest,
     outcome_matches (ci_idx input) (ew_node_ref (sm_work sm_op)) (ew_occurrence (sm_work sm_op))
       (oa_total acc_rest sm_op)) ->
  outcome_matches (ci_idx input) (ew_node_ref current) (ew_occurrence current) o.
Proof.
  ...
  - exact (leaf_stored_matches (ci_idx input) (ew_node_ref current) (ew_expr_ref current)
             (ew_occurrence current) (ew_expr current) ci (ew_view_exact current)
             (ew_occ_role current) Hconst).
  - (* SCConvOk: the index EQUALS [conv_outcome] over the operand (EOOk opf) with a succeeding convert_const *)
    enough (Hm : outcome_matches (ci_idx input) (ew_node_ref current) (ew_occurrence current)
                   (conv_outcome tnft (ew_expr_ref current) (cw_target_ref (cs_conversion step))
                      (ew_expr_ref (proj1_sig (cw_operand_work (cs_conversion step))))
                      (oa_total acc_rest (cs_operand_suffix step)))).
  ...
  - (* SCConvFail *)
    ...
    { revert Hm. rewrite Hop. cbn [conv_outcome]. rewrite Hconv. exact (fun H => H). }
    apply conv_step_matches; assumption.
  - (* SCChildFail: operand failed, so [conv_outcome] is [EOChildFail] *)
    enough (Hm : outcome_matches (ci_idx input) (ew_node_ref current) (ew_occurrence current)
                   (conv_outcome tnft (ew_expr_ref current) (cw_target_ref (cs_conversion step))
                      (ew_expr_ref (proj1_sig (cw_operand_work (cs_conversion step))))
                      (oa_total acc_rest (cs_operand_suffix step)))).
    { revert Hm. unfold conv_outcome.
      destruct (oa_total acc_rest (cs_operand_suffix step)) as [opf| | ];
...
(* a retained [WorkMember] IS a [SuffixMember] of the whole [ewf_items] (its own membership proof). *)
...
Definition fot_map {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (ot : ForestOutcomeTable forest tnft) : GoIndex.NodeKeyMapBase.t (ExprOutcome p) := oa_map (fot_acc ot).
Definition fot_dom {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (ot : ForestOutcomeTable forest tnft) :
    outcome_dom_exact (map (fun w0 => (ew_node_ref w0, ew_occurrence w0)) (ewf_items forest)) (fot_map ot) :=
  oa_domain (fot_acc ot).
...
Definition fot_present {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
...
Proof.
  intros Hin Hv.
  destruct (GoIndex.NodeKeyMapBase.find (GoIndex.Snap.node_ref_key r) (fot_map ot)) as [w|] eqn:E;
    [exfalso | reflexivity].
  destruct (fot_dom ot (GoIndex.Snap.node_ref_key r) (ltac:(rewrite E; discriminate)))
    as [r' [occ' [e' [Hin' [Hk' Hv']]]]].
  apply in_map_iff in Hin'. destruct Hin' as [w0 [Hpair Hinw0]].
  injection Hpair as Hnr Hocc. subst r' occ'.
  assert (Hrr : ew_node_ref w0 = r) by (apply GoIndex.Snap.node_ref_key_inj; exact Hk').
  assert (Hoc : ew_occurrence w0 = occ).
  { rewrite (prog_visit_occ_is_source p (ew_node_ref w0) (ew_occurrence w0) (ci_in_prog (ew_in_visit w0))).
    rewrite Hrr, (prog_visit_occ_is_source p r occ (ci_in_prog Hin)). reflexivity. }
...
(** §5/§9.3 the RETAINED direct cause, PROJECTED by inverting the member's [StepCause] (axiom-free — the outcome
    ...
    [oa_total acc_rest], NOT a raw re-derived key. *)
Lemma StepCause_convfail_inv {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (current : ExprWork input) (rest : list (ExprWork input)) (acc_rest : OutcomeAccumulator forest tnft rest)
    er2 tr2 opr2 t ci :
  StepCause forest tnft current rest acc_rest (EOConvFail er2 tr2 opr2 t ci) ->
  exists ts x (step : ConversionStep forest current rest ts x) opf,
       ew_expr current = EConvert ts x
    /\ oa_total acc_rest (cs_operand_suffix step) = EOOk opf
    /\ er2 = ew_expr_ref current
    /\ tr2 = cw_target_ref (cs_conversion step)
    /\ opr2 = ew_expr_ref (proj1_sig (cw_operand_work (cs_conversion step)))
    /\ t = tnf_type (type_name_fact_at_table tnft (cw_target_ref (cs_conversion step)))
    /\ ci = ef_const_status opf
    /\ convert_const t ci = None.
Proof.
  intro Hsc. remember (EOConvFail er2 tr2 opr2 t ci) as ov eqn:Hov.
...
Lemma StepCause_childfail_inv {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (current : ExprWork input) (rest : list (ExprWork input)) (acc_rest : OutcomeAccumulator forest tnft rest) :
  StepCause forest tnft current rest acc_rest EOChildFail ->
  exists ts x (step : ConversionStep forest current rest ts x),
    ...
    /\ outcome_is_fail (oa_total acc_rest (cs_operand_suffix step)).
Proof.
  intro Hsc. remember EOChildFail as ov eqn:Hov.
  destruct Hsc as [ ci0 Hnone Hconst | ts x step opf tc Hcur He Hop Hconv
                  | ts x step opf Hcur He Hop Hconv | ts x step Hcur He Hfail ];
    try discriminate Hov.
...
Lemma StepCause_ok_conv_inv {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
    (current : ExprWork input) (rest : list (ExprWork input)) (acc_rest : OutcomeAccumulator forest tnft rest)
    ts0 x0 f :
  ...
  exists (step : ConversionStep forest current rest ts0 x0) opf tc,
    ...
    /\ f = mkExprFact (CITyped (tnf_type (type_name_fact_at_table tnft (cw_target_ref (cs_conversion step)))) tc)
             (use_resolved_of_ci (expr_ref_role (ew_expr_ref current))
                (CITyped (tnf_type (type_name_fact_at_table tnft (cw_target_ref (cs_conversion step)))) tc)).
Proof.
  intros Hconvcur Hsc. remember (EOOk f) as ov eqn:Hov.
  destruct Hsc as [ ci0 Hnone Hconst | ts x step opf tc Hcur He Hop Hconv
                  | ts x step opf Hcur He Hop Hconv | ts x step Hcur He Hfail ];
    try discriminate Hov.
...
(* the SPECIFICATION expression-diagnostic report: the declarative per-occurrence [occ_expr_diags] over the
   one-pass annotated stream.  The PRODUCTION report is the retained [ExpressionDiagnostics] object's list (the
   TOTAL projection of the retained annotated work object over the [ForestOutcomeTable]), proved equal to THIS
   ([expression_diagnostics_eq_spec] / [ep_diags_eq_expr_diags]). *)
...
(* the align projection of a retained annotated member: its (node,occurrence) pair with its context.  A NAMED
   constant (not an inline lambda) so [map ew_pair_ctx …] is IDENTICAL across the object/lemmas that reference
   it — no cross-definition lambda-elaboration divergence. *)
Definition ew_pair_ctx {p} {input : CompilationInput p} {forest : ExprWorkForest input}
    (x : WorkMember forest * list (GoIndex.ExprRef p))
  : (GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) * list (GoIndex.ExprRef p) :=
  ((ew_node_ref (proj1_sig (fst x)), ew_occurrence (proj1_sig (fst x))), snd x).
...
Lemma flat_map_occ_is_expr_filter {p}
    (l : list ((GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) * list (GoIndex.ExprRef p))) :
  flat_map (fun rc => if occ_is_expr (fst rc) then [ (fst rc, snd rc) ] else []) l
  = filter (fun rc => occ_is_expr (fst rc)) l.
Proof.
  induction l as [|[ro c] l IH]; [reflexivity|].
  cbn [flat_map filter fst snd]. destruct (occ_is_expr ro); cbn [app]; rewrite IH; reflexivity.
Qed.
...
(** ═══ §7/§2.6 THE ONE RETAINED ANNOTATED WORK FOREST OBJECT ═══ a proof-CARRYING record over the exact retained
    [forest].  Its [aewf_items] are the retained [WorkMember]s — EXACTLY the forest's items, once each, in forest
    order ([aewf_members]) — each paired with its enclosing-conversion context.  The load-bearing [aewf_diag_fold]
    ...
    [build_annotated_work_forest] from the forest's own [ewf_blocks] (NO [build_forest_blocks]/[build_expr_work_
...
Record AnnotatedExprWorkForest {p} {input : CompilationInput p} (forest : ExprWorkForest input) : Type :=
  mkAnnotatedExprWorkForest {
  aewf_items : list (WorkMember forest * list (GoIndex.ExprRef p)) ;
  aewf_members : map (fun x => proj1_sig (fst x)) aewf_items = ewf_items forest ;
  aewf_diag_fold :
    forall (X : Type) (d : (WorkMember forest * list (GoIndex.ExprRef p)) -> list X)
           (dr : (GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) -> list (GoIndex.ExprRef p) -> list X),
      (forall wm c, d (wm, c) = dr (ew_node_ref (proj1_sig wm), ew_occurrence (proj1_sig wm)) c) ->
      (forall ro c, In ro (concat (ci_blocks input)) -> GoIndex.view_expr (snd ro) = None -> dr ro c = []) ->
      flat_map d aewf_items
      = flat_map (fun rc => dr (fst rc) (snd rc)) (annotate_program (ci_idx input)) ;
  aewf_context_sound : forall x er, In x aewf_items -> In er (snd x) ->
    is_conversion_occ (GoIndex.Snap.source_occurrence_of_ref (GoIndex.erase_ref er)) = true
    /\ Pos.lt (GoIndex.Snap.node_ref_local (GoIndex.erase_ref er))
              (GoIndex.Snap.node_ref_local (ew_node_ref (proj1_sig (fst x))))
    /\ Pos.le (GoIndex.Snap.node_ref_local (ew_node_ref (proj1_sig (fst x))))
              (GoIndex.Snap.node_subtree_end (ci_idx input) (GoIndex.erase_ref er)) ;
  aewf_context_same_file : forall x, In x aewf_items ->
    Forall (fun er => GoIndex.Snap.node_ref_file (GoIndex.erase_ref er)
                      = GoIndex.Snap.node_ref_file (ew_node_ref (proj1_sig (fst x)))) (snd x) ;
  aewf_context_nearest_first : forall x, In x aewf_items ->
    StronglySorted (fun a b => Pos.lt (GoIndex.Snap.node_ref_local (GoIndex.erase_ref b))
                                       (GoIndex.Snap.node_ref_local (GoIndex.erase_ref a))) (snd x) ;
  aewf_context_nodup : forall x, In x aewf_items -> NoDup (snd x)
}.
Arguments mkAnnotatedExprWorkForest {p input forest} _ _ _ _ _ _ _.
Arguments aewf_items {p input forest} _.  Arguments aewf_members {p input forest} _.
Arguments aewf_diag_fold {p input forest} _.  Arguments aewf_context_sound {p input forest} _.
Arguments aewf_context_same_file {p input forest} _.  Arguments aewf_context_nearest_first {p input forest} _.
Arguments aewf_context_nodup {p input forest} _.
...
Definition build_annotated_work_forest {p} {input : CompilationInput p} (forest : ExprWorkForest input)
  : AnnotatedExprWorkForest forest.
Proof.
  destruct (build_forest_awork_blocks forest (ci_blocks input) (ewf_blocks forest)
              (ewf_blocks_exact forest) (ewf_concat_mem forest)) as [aw [Hmem Hfoldb]].
  assert (Hprog : forall (X : Type) (d : (WorkMember forest * list (GoIndex.ExprRef p)) -> list X)
           (dr : (GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) -> list (GoIndex.ExprRef p) -> list X),
      (forall wm c, d (wm, c) = dr (ew_node_ref (proj1_sig wm), ew_occurrence (proj1_sig wm)) c) ->
      (forall ro c, In ro (concat (ci_blocks input)) -> GoIndex.view_expr (snd ro) = None -> dr ro c = []) ->
      flat_map d aw = flat_map (fun rc => dr (fst rc) (snd rc)) (annotate_program (ci_idx input))).
  { intros X d dr Hag Hemp. rewrite (Hfoldb X d dr Hag Hemp).
    unfold annotate_program. rewrite (ci_blocks_ok input). reflexivity. }
  assert (Hag : forall (wm : WorkMember forest) (c : list (GoIndex.ExprRef p)),
      [ ((ew_node_ref (proj1_sig wm), ew_occurrence (proj1_sig wm)), c) ]
      = (if occ_is_expr (ew_node_ref (proj1_sig wm), ew_occurrence (proj1_sig wm))
         then [ ((ew_node_ref (proj1_sig wm), ew_occurrence (proj1_sig wm)), c) ] else [])).
  { intros wm c. rewrite (occ_is_expr_true (ew_node_ref (proj1_sig wm)) (ew_occurrence (proj1_sig wm))
        (ew_expr (proj1_sig wm)) (ew_view_exact (proj1_sig wm))). reflexivity. }
  assert (Hemp : forall (ro : GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) (c : list (GoIndex.ExprRef p)),
      In ro (concat (ci_blocks input)) -> GoIndex.view_expr (snd ro) = None ->
      (if occ_is_expr ro then [ (ro, c) ] else []) = nil).
  { intros [r occ] c _ Hv. cbn [snd] in Hv. rewrite (occ_is_expr_false r occ Hv). reflexivity. }
  assert (Halign : map ew_pair_ctx aw
                   = filter (fun rc => occ_is_expr (fst rc)) (annotate_program (ci_idx input))).
  { rewrite <- (flat_map_sing ew_pair_ctx aw).
    rewrite (Hprog (((GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) * list (GoIndex.ExprRef p))%type)
               (fun x => [ ew_pair_ctx x ])
               (fun ro c => if occ_is_expr ro then [ (ro, c) ] else []) Hag Hemp).
    cbn beta. exact (flat_map_occ_is_expr_filter (annotate_program (ci_idx input))). }
  assert (Hin : forall x, In x aw ->
     In ((ew_node_ref (proj1_sig (fst x)), ew_occurrence (proj1_sig (fst x))), snd x)
        (annotate_program (ci_idx input))).
  { intros x Hx.
    assert (Hxm : In (ew_pair_ctx x) (map ew_pair_ctx aw)) by (apply in_map; exact Hx).
    rewrite Halign in Hxm. apply filter_In in Hxm. exact (proj1 Hxm). }
  refine (mkAnnotatedExprWorkForest aw _ Hprog _ _ _ _).
  - exact (eq_trans Hmem (eq_sym (ewf_flat forest))).
  - intros x er Hx Her.
    exact (annotate_program_ctx_sound (ci_idx input)
             (ew_node_ref (proj1_sig (fst x)), ew_occurrence (proj1_sig (fst x))) (snd x) er (Hin x Hx) Her).
  - intros x Hx.
    exact (proj1 (annotate_program_ctx_wf (ci_idx input)
             (ew_node_ref (proj1_sig (fst x)), ew_occurrence (proj1_sig (fst x))) (snd x) (Hin x Hx))).
  - intros x Hx.
    exact (proj1 (proj2 (annotate_program_ctx_wf (ci_idx input)
             (ew_node_ref (proj1_sig (fst x)), ew_occurrence (proj1_sig (fst x))) (snd x) (Hin x Hx)))).
  - intros x Hx.
    exact (proj2 (proj2 (annotate_program_ctx_wf (ci_idx input)
             (ew_node_ref (proj1_sig (fst x)), ew_occurrence (proj1_sig (fst x))) (snd x) (Hin x Hx)))).
Qed.
...
(* the retained annotated object's occurrence/context alignment: the members' (node,occ,ctx) reproduce the
   ...
   carried [aewf_diag_fold] (no rebuild). *)
Lemma aewf_align_eq {p} {input : CompilationInput p} {forest : ExprWorkForest input}
    (aw : AnnotatedExprWorkForest forest) :
  map ew_pair_ctx (aewf_items aw)
  = filter (fun rc => occ_is_expr (fst rc)) (annotate_program (ci_idx input)).
Proof.
  assert (Hag : forall (wm : WorkMember forest) (c : list (GoIndex.ExprRef p)),
      [ ((ew_node_ref (proj1_sig wm), ew_occurrence (proj1_sig wm)), c) ]
  ...
  assert (Hemp : forall (ro : GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) (c : list (GoIndex.ExprRef p)),
      ...
      (if occ_is_expr ro then [ (ro, c) ] else []) = nil).
  { intros [r occ] c _ Hv. cbn [snd] in Hv. rewrite (occ_is_expr_false r occ Hv). reflexivity. }
  rewrite <- (flat_map_sing ew_pair_ctx (aewf_items aw)).
  rewrite (aewf_diag_fold aw (((GoIndex.Snap.NodeRef p * GoIndex.SourceOccurrence) * list (GoIndex.ExprRef p))%type)
             (fun x => [ ew_pair_ctx x ])
             (fun ro c => if occ_is_expr ro then [ (ro, c) ] else []) Hag Hemp).
  cbn beta. exact (flat_map_occ_is_expr_filter (annotate_program (ci_idx input))).
Qed.
...
(* §8.2 the diagnostic projection over the retained annotated object EQUALS the source spec over [annotate_program]
   — proved directly from the object's own [aewf_diag_fold] (agreement = [forest_awork_diags_eq]; skip =
   [occ_expr_diags_nonexpr]).  The phase's diagnostics CONSUME this object, never a recomputed annotation. *)
Lemma expression_diagnostics_eq_spec {p} {input : CompilationInput p} {forest : ExprWorkForest input}
    {tnft : TypeNameFactTable p} (ot : ForestOutcomeTable forest tnft) (aw : AnnotatedExprWorkForest forest) :
  flat_map (forest_awork_diags ot) (aewf_items aw)
  = flat_map (fun roc => occ_expr_diags (ci_idx input) (snd roc) (fst roc)) (annotate_program (ci_idx input)).
Proof.
  apply (aewf_diag_fold aw (DiagnosticReason p)
           (forest_awork_diags ot) (fun ro c => occ_expr_diags (ci_idx input) c ro)).
  - intros wm c. exact (forest_awork_diags_eq ot wm c).
  - intros ro c _ Hv. exact (occ_expr_diags_nonexpr (ci_idx input) c ro Hv).
Qed.
...
(** §8.2 the FOREST/OUTCOME-INDEXED diagnostics OBJECT — the stored diagnostic list PLUS a proof it is the
    ...
    [AnnotatedExprWorkForest] and [ot], so a foreign annotated object / diagnostic list cannot be inserted. *)
Record ExpressionDiagnostics {p} {input : CompilationInput p} {forest : ExprWorkForest input}
  ...
  ed_is_diags : ed_diags = flat_map (forest_awork_diags ot) (aewf_items aw)
}.
Arguments mkExpressionDiagnostics {p input forest aw tnft ot} _ _.
Arguments ed_diags {p input forest aw tnft ot} _.  Arguments ed_is_diags {p input forest aw tnft ot} _.

Definition build_expression_diagnostics {p} {input : CompilationInput p} {forest : ExprWorkForest input}
    (aw : AnnotatedExprWorkForest forest)
    {tnft : TypeNameFactTable p} (ot : ForestOutcomeTable forest tnft) : ExpressionDiagnostics aw ot :=
  mkExpressionDiagnostics (flat_map (forest_awork_diags ot) (aewf_items aw)) eq_refl.
...
(** §9/§10 THE ONE INTRINSIC EXPRESSION PHASE — a DEPENDENT CHAIN of retained objects: each later component is
    ...
    diagnostics came from another forest/table is UNREPRESENTABLE (type mismatch).  NO provenance-equality field
    exists: the causal chain is the DEPENDENT TYPES themselves.
      ...
      · [ep_awork] : [AnnotatedExprWorkForest ep_work]            (the retained annotated object of [ep_work]);
    ...
    [ep_work]/[ep_tnft] DEFINITIONALLY the retained-input builders (no stored equality needed). *)
...
Arguments mkExpressionPhase {p input} _ _ _ _ _ _.
...
Lemma retained_convfail_diag {p} {input : CompilationInput p} {forest : ExprWorkForest input} {tnft}
  ...
  exists (wma : WorkMember forest) (outer : list (GoIndex.ExprRef p)),
       In (wma, outer) (aewf_items aw)
...
Proof.
  ...
  - unfold ep_diags. rewrite (ed_is_diags (ep_diag ph)).
    exact (expression_diagnostics_eq_spec (ep_ot ph) (ep_awork ph)).
Qed.
...
(** §7 THE [aewf_spec_exact] SURFACE (placed here, where [erase_annot]/[annotate_source] are defined): erasing
    ...
    occurrence/context alignment ([aewf_align_eq]) composed with the erasure simulation ([annotate_program_erased])
...
Lemma annotated_forest_erased_source {p} {input : CompilationInput p}
    (forest : ExprWorkForest input) (aw : AnnotatedExprWorkForest forest) :
  map (fun x => erase_annot (ew_pair_ctx x)) (aewf_items aw)
  = filter (fun kc => match GoIndex.view_expr (snd (fst kc)) with Some _ => true | None => false end)
           (annotate_source (prog_files p)).
Proof.
  etransitivity.
  { symmetry. apply (map_map ew_pair_ctx erase_annot (aewf_items aw)). }
  rewrite (aewf_align_eq aw).
  rewrite (map_filter_comm erase_annot
             (fun kc => match GoIndex.view_expr (snd (fst kc)) with Some _ => true | None => false end)
             (fun rc => occ_is_expr (fst rc)) (annotate_program (ci_idx input))
             (fun a => ltac:(destruct a as [[r occ] ctx]; cbn [erase_annot occ_is_expr fst snd]; reflexivity))).
  rewrite (annotate_program_erased (ci_idx input)). reflexivity.
Qed.
...
(* the core's raw fold IS the canonical [semantic_diagnostics] — for ANY core, because a phase's diagnostics are
   determined by its input ([ep_diags_eq_expr_diags] is universal in the phase). *)
Lemma ec_raw_semantic {p} (core : ElaborationCore p) :
  bucket_flatten (node_keyed (ec_raw core)) ++ pkg_primary (ec_raw core)
  = semantic_diagnostics p (ec_idx core).
Proof.
  unfold ec_raw, ec_buckets, ec_idx.
  rewrite (ep_diags_eq_expr_diags (ec_input core) (ec_phase core)). reflexivity.
Qed.
...
Lemma ec_diags_eq_elaboration {p} (core : ElaborationCore p) :
  ec_diags core = elaboration_diagnostics p (ec_idx core).
Proof.
  unfold ec_diags, ec_plan, ec_layout, ec_buckets.
  rewrite (ec_raw_semantic core). exact (command_plan_diags_eq p (ec_ip core)).
Qed.

(** ═══ §4 (REPAIR 14) THE ACCEPTED VIEW ═══ [ElaborationFacts] built BY PROJECTING the retained core, given the
...
Definition core_facts {p} (core : ElaborationCore p) (Hnil : ec_diags core = nil)
  : ElaborationFacts p (ec_ip core) :=
  let He : elaboration_diagnostics p (ec_idx core) = nil :=
    eq_trans (eq_sym (ec_diags_eq_elaboration core)) Hnil in
  mkElaborationFacts
    (feft_table (ep_eft (ec_phase core)))      (* the phase's OWN retained expression-fact table object *)
    (ep_tnft (ec_phase core))                  (* the phase's OWN retained type-name table object *)
    (ec_buckets core)
    (prog_package_refs_present (ec_idx core))
    (prog_package_refs_bucket_len (ec_idx core))
    (prog_package_refs_belongs (ec_idx core))
    (elaboration_no_diags_source_valid p (ec_idx core) He)
    (elaboration_no_diags_preflight p (ec_idx core) He)
    (ec_layout core) eq_refl
    (ec_plan core) (fresh_build_plan_of_buckets p (ec_idx core)).
...
Inductive ElaborationDecision {p} (core : ElaborationCore p) : Type :=
| ElaborationAccepted (Hnil : ec_diags core = nil)
| ElaborationRejected (Hne : ec_diags core <> nil).
Arguments ElaborationAccepted {p core} _.
Arguments ElaborationRejected {p core} _.
...
(** ═══ §3 (REPAIR 14) THE ELABORATION RETAINS ITS CORE ═══ the whole causal object survives BOTH branches, and
    the accept/reject decision is INDEXED BY it.  [pe_indexed] and [pe_result] remain as total PROJECTIONS, so
    ...
    [ProgramElaboration] cannot be built without the core that justifies it. *)
Record ProgramElaboration (p : GoProgram) : Type := mkProgramElaboration {
  pe_core     : ElaborationCore p;
  pe_decision : ElaborationDecision pe_core
}.
Arguments mkProgramElaboration {p} _ _.
Arguments pe_core {p} _.
Arguments pe_decision {p} _.

Definition pe_indexed {p} (pe : ProgramElaboration p) : GoIndex.IndexedProgram p := ec_ip (pe_core pe).

Definition pe_result {p} (pe : ProgramElaboration p) : ElaborationResult p (pe_indexed pe) :=
  match pe_decision pe with
  | ElaborationAccepted Hnil => ElaborationOK (core_facts (pe_core pe) Hnil)
  | ElaborationRejected Hne  => ElaborationFailed (ec_diags (pe_core pe)) Hne
  end.
...
Definition decision_of_core {p} (core : ElaborationCore p) : ElaborationDecision core :=
  match list_is_nil (ec_diags core) with
  | left He   => ElaborationAccepted He
  | right Hne => ElaborationRejected Hne
  end.
...
Definition elaborate_at {p : GoProgram} (ip : GoIndex.IndexedProgram p) : ProgramElaboration p :=
  let core := build_elaboration_core p ip in
  mkProgramElaboration core (decision_of_core core).

Definition elaborate (p : GoProgram) : ProgramElaboration p :=
  elaborate_at (GoIndex.index_program p).
...
Theorem core_seals_tnfacts {p} (core : ElaborationCore p) (Hnil : ec_diags core = nil) :
  ef_type_name_facts (core_facts core Hnil) = ep_tnft (ec_phase core).
Proof. reflexivity. Qed.

Theorem core_seals_facts {p} (core : ElaborationCore p) (Hnil : ec_diags core = nil) :
  ef_expr_facts (core_facts core Hnil) = feft_table (ep_eft (ec_phase core)).
Proof. reflexivity. Qed.

(* and the retained-INPUT provenance, for a core the elaborator actually built: its type-name table IS the
   exact [build_type_name_fact_table] of the core's own retained input. *)
Corollary built_core_tnfacts_from_input (p : GoProgram) (ip : GoIndex.IndexedProgram p) Hnil :
  ef_type_name_facts (core_facts (build_elaboration_core p ip) Hnil)
  = build_type_name_fact_table (ec_input (build_elaboration_core p ip)).
Proof. reflexivity. Qed.
...
Proof.
  unfold elaborate, elaborate_at, decision_of_core, pe_result; cbn [pe_core pe_decision]; cbv zeta.
...
Proof.
  unfold elaborate, elaborate_at, pe_result, decision_of_core; cbn [pe_core pe_decision]; cbv zeta.
...
Lemma program_elaboration_eta {p} (a : ProgramElaboration p) :
  a = mkProgramElaboration (pe_core a) (pe_decision a).
...
(** ═══ §6/§9 (REPAIR 14) THE CAPABILITY RETAINS THE CORE ═══ the whole-equation scaffolding that used to lift a
    [pe_result] fact to [elaborate p = mkProgramElaboration ip (ElaborationOK facts)] is DELETED along with
    [cp_prov]: it existed only to reconstruct a discarded whole result and to serve as provenance-by-equality.
    A [CompilableProgram] now RETAINS the exact accepted [ElaborationCore] — with it, the input, the phase, the
    work forest and index, the outcome table and its trace, the annotation, the fact tables and the diagnostics.
...
Definition elaboration_ok_core (p : GoProgram) (H : GoCompile p)
  : { core : ElaborationCore p | ec_diags core = nil } :=
  let core := build_elaboration_core p (GoIndex.index_program p) in
  match list_is_nil (ec_diags core) with
  | left He   => exist _ core He
  | right Hne =>
      False_rect _ (Hne (eq_trans (ec_diags_eq_elaboration core)
        (proj2 (elaboration_diagnostics_nil_iff_GoCompile p
                  (GoIndex.indexed_syntax (GoIndex.index_program p))) H)))
  end.

Record CompilableProgram : Type := mkCompilable {
  cp_program : GoProgram;
  cp_core    : ElaborationCore cp_program;      (* THE retained whole elaboration *)
  cp_nil     : ec_diags cp_core = nil           (* the accepted evidence, over THAT exact core *)
}.

(* every public component is a projection of the retained core — never a re-elaboration. *)
Definition cp_index (cp : CompilableProgram) : GoIndex.IndexedProgram (cp_program cp) := ec_ip (cp_core cp).
Definition cp_input (cp : CompilableProgram) : CompilationInput (cp_program cp) := ec_input (cp_core cp).
Definition cp_phase (cp : CompilableProgram) : ExpressionPhase (cp_input cp) := ec_phase (cp_core cp).
Definition cp_work  (cp : CompilableProgram) : ExprWorkForest (cp_input cp) := ep_work (cp_phase cp).
Definition cp_trace (cp : CompilableProgram) := ep_ot (cp_phase cp).
Definition cp_diags (cp : CompilableProgram) : list (DiagnosticReason (cp_program cp)) := ec_diags (cp_core cp).
Definition cp_layout (cp : CompilableProgram) := ec_layout (cp_core cp).
Definition cp_plan   (cp : CompilableProgram) := ec_plan (cp_core cp).

Definition cp_facts (cp : CompilableProgram) : ElaborationFacts (cp_program cp) (cp_index cp) :=
  core_facts (cp_core cp) (cp_nil cp).
...
Theorem compilable_retains_phase : forall cp : CompilableProgram, cp_phase cp = ec_phase (cp_core cp).
Proof. reflexivity. Qed.

Theorem compilable_retains_expr_facts : forall cp : CompilableProgram,
  ef_expr_facts (cp_facts cp) = feft_table (ep_eft (cp_phase cp)).
Proof. reflexivity. Qed.

Theorem compilable_retains_tnfacts : forall cp : CompilableProgram,
  ef_type_name_facts (cp_facts cp) = ep_tnft (cp_phase cp).
Proof. reflexivity. Qed.
...
(** the production compiler READS the retained core's decision and PASSES THE CORE THROUGH.  There is no
    [elaborate p = a] provenance argument any more: the accepted [CompilableProgram] holds the very
    [ElaborationCore] whose diagnostics the decision judged, so success needs no equation to a rerun and failure
    carries that same core's diagnostics.  One match, no convoy, nothing rebuilt. *)
Definition outcome_of_elaboration (p : GoProgram) (a : ProgramElaboration p) : CompileOutcome p :=
  match pe_decision a with
  | ElaborationAccepted Hnil => CompiledOk (mkCompilable p (pe_core a) Hnil) eq_refl
  | ElaborationRejected Hne  => CompileFailed (mkCompileFailure (ec_diags (pe_core a)) Hne)
  end.

Definition go_compile (p : GoProgram) : CompileOutcome p :=
  outcome_of_elaboration p (elaborate p).

(** the ONE reduction fact for the whole pipeline: [go_compile] IS the decision on the built core.  Every
...
Lemma go_compile_on_core : forall p,
  go_compile p =
    (match list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p))) with
     | left He   => CompiledOk (mkCompilable p (build_elaboration_core p (GoIndex.index_program p)) He) eq_refl
     | right Hne => CompileFailed (mkCompileFailure
                      (ec_diags (build_elaboration_core p (GoIndex.index_program p))) Hne)
     end).
Proof.
  intro p. unfold go_compile, outcome_of_elaboration, elaborate, elaborate_at, decision_of_core.
  cbn [pe_core pe_decision]; cbv zeta.
  destruct (list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p)))); reflexivity.
Qed.

(** the same reduction for the elaboration result, so a [pe_result] hypothesis and a [go_compile] goal are
    discharged by ONE [destruct] of the same scrutinee. *)
Lemma pe_result_on_core : forall p,
  pe_result (elaborate p) =
    (match list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p))) with
     | left He   => ElaborationOK (core_facts (build_elaboration_core p (GoIndex.index_program p)) He)
     | right Hne => ElaborationFailed (ec_diags (build_elaboration_core p (GoIndex.index_program p))) Hne
     end).
Proof.
  intro p. unfold elaborate, elaborate_at, pe_result, decision_of_core.
  cbn [pe_core pe_decision]; cbv zeta.
  destruct (list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p)))); reflexivity.
Qed.

(** admissibility forces the accepted branch: the built core's diagnostics ARE [elaboration_diagnostics]. *)
Lemma core_diags_nil_of_valid : forall p, GoCompile p ->
  ec_diags (build_elaboration_core p (GoIndex.index_program p)) = nil.
Proof.
  intros p Hv.
  exact (eq_trans (ec_diags_eq_elaboration (build_elaboration_core p (GoIndex.index_program p)))
           (proj2 (elaboration_diagnostics_nil_iff_GoCompile p
                     (GoIndex.indexed_syntax (GoIndex.index_program p))) Hv)).
Qed.
...
Proof.
  intros p Hvalid. rewrite go_compile_on_core.
  destruct (list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p)))) as [He|Hne].
  - eexists; eexists; reflexivity.
...
(** DECLARATIVE AND EXECUTABLE EXACTNESS.  The three diagnostic layers each have an emptiness/equivalence
    characterization, and the two-branch command structure is pinned:
      ...
      I. [compilable_retains_phase]/[compilable_retains_expr_facts]/[cp_build_plan] : retention of the core.
...
Lemma go_compile_projects_elaborate : forall p,
  go_compile p = outcome_of_elaboration p (elaborate p).
Proof. intro p. reflexivity. Qed.

(** RETENTION: the capability holds the exact [ElaborationCore]; program/index/facts/phase are PROJECTIONS of
...
(** the witness builder: from validity, [elaboration_ok_core] runs the elaborator ONCE and returns that exact
    ...
    to rebuild and no provenance equation to discharge.  [cp_program] stays a direct first-field projection
    ([= p]) so rendering/emission never reduce the opaque, vm-compute-unfriendly elaboration.  This is the SAME
    object [go_compile]'s success value carries — the two artifacts are built ONE way. *)
Definition compilable_of_valid (p : GoProgram) (H : GoCompile p) : CompilableProgram :=
  let s := elaboration_ok_core p H in
  mkCompilable p (proj1_sig s) (proj2_sig s).
...
Proof.
  intros p Hf Hpf. rewrite go_compile_on_core.
  destruct (list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p)))) as [He|Hne].
  - exfalso.
    assert (Hgc : GoCompile p).
    { apply (proj1 (elaboration_diagnostics_nil_iff_GoCompile p
               (GoIndex.indexed_syntax (GoIndex.index_program p)))).
      exact (eq_trans (eq_sym (ec_diags_eq_elaboration (build_elaboration_core p (GoIndex.index_program p)))) He). }
  ...
  - cbn [legacy_compile_class cfail_diags]. unfold legacy_class_of_diags.
    rewrite (ec_diags_eq_elaboration (build_elaboration_core p (GoIndex.index_program p))), (elaboration_diagnostics_eq_semantic p _ Hpf),
...
Proof.
  intro p. unfold go_compile_class. rewrite go_compile_on_core.
  destruct (list_is_nil (ec_diags (build_elaboration_core p (GoIndex.index_program p)))) as [He|Hne].
  -
    assert (Hgc : GoCompile p).
    { apply (proj1 (elaboration_diagnostics_nil_iff_GoCompile p
  ...
  - cbn [legacy_compile_class cfail_diags]. unfold legacy_class_of_diags.
    rewrite (ec_diags_eq_elaboration (build_elaboration_core p (GoIndex.index_program p))).
    ...
    + rewrite (elaboration_diagnostics_eq_semantic p _ Ep), existsb_build_output_semantic,
      ...
      assert (Hnv : ~ GoCompile p) by (intro Hv; exact (Hne (core_diags_nil_of_valid p Hv))).
...
(* REPAIR 14 — the REJECTED side on a concrete program: the failure value carries the diagnostics of the very
   [ElaborationCore] the decision judged ([ec_diags (pe_core (elaborate …))]), not a recomputed list.  The
...
Theorem over_program_failure_carries_core_diags :
  exists Hne,
    go_compile over_program
    = CompileFailed (mkCompileFailure (ec_diags (pe_core (elaborate over_program))) Hne).
Proof.
  rewrite go_compile_on_core.
  destruct (list_is_nil (ec_diags (build_elaboration_core over_program (GoIndex.index_program over_program))))
    as [He|Hne].
  - exfalso. apply over_program_no_compile.
    apply (proj1 (elaboration_diagnostics_nil_iff_GoCompile over_program
             (GoIndex.indexed_syntax (GoIndex.index_program over_program)))).
    exact (eq_trans (eq_sym (ec_diags_eq_elaboration
             (build_elaboration_core over_program (GoIndex.index_program over_program)))) He).
...
(* §12/§6 — the innermost int8(300) convfail projects its RETAINED cause from the corrected outcome table: the
   member's [total_forest_outcome_cause] destructs to the exact insertion [StepCause], which (inverted) yields the
   exact [ConversionStep], the operand outcome read THROUGH the exact operand [SuffixMember] via [oa_total acc_rest]
   ([= EOOk opf]), ONE rejecting [convert_const], and an [EOConvFail] naming the exact operand ref.  No post-hoc
   [ConversionWork] built after the table result — the cause is the one the fold retained. *)
Theorem deep_fail_innermost_convfail :
  ...
  exists (wm : WorkMember (ep_work (build_expression_phase input)))
         (rest : list (ExprWork input))
         (acc_rest : OutcomeAccumulator (ep_work (build_expression_phase input))
                       (ep_tnft (build_expression_phase input)) rest)
         (step : ConversionStep (ep_work (build_expression_phase input)) (proj1_sig wm) rest
    ...
    (* the resolved target type of the failing conversion IS the exact predeclared-context table query *)
    /\ t = tnf_type (type_name_fact_at_table (ep_tnft (build_expression_phase input))
...
Theorem deep_fail_innermost_diag :
  ...
  exists (wm : WorkMember (ep_work phase))
         ...
         opf t (wma : WorkMember (ep_work phase)) (outer : list (GoIndex.ExprRef deep_fail_program)),
       total_forest_outcome_at ot wm
         = EOConvFail (ew_expr_ref (proj1_sig wm)) (cw_target_ref (cs_conversion step))
             (ew_expr_ref (proj1_sig (cw_operand_work (cs_conversion step)))) t (ef_const_status opf)
       (* the resolved target type of the diagnostic IS the exact predeclared-context table query for that ref *)
    /\ t = tnf_type (type_name_fact_at_table (ep_tnft phase) (cw_target_ref (cs_conversion step)))
    /\ oa_total acc_rest (cs_operand_suffix step) = EOOk opf
    /\ total_forest_outcome_at ot (proj1_sig (cs_operand_suffix step)) = EOOk opf
    /\ convert_const t (ef_const_status opf) = None
...
(* §9.3 the OPERAND CLOSURE at a valid conversion occurrence: locate the member, project the RETAINED cause, and read
   the operand's closure — the operand's [EOOk opf] in the RETAINED tail accumulator is the SAME [EOOk opf] the FINAL
   table shows at the operand [WorkMember].  Its STATEMENT returns ONLY that operand tail/final [EOOk opf] + the
   query equality; the exact current final [ExprFact], the target fact, and the ONE [convert_const] success (which the
...
(* NOTE ([deep_nested_ok_closure_at] is an OPERAND-CLOSURE-ONLY corollary): it exposes only the operand's
   tail/final [EOOk opf] + query equality.  The EXACT accepted valid-chain success evidence — which additionally
   STATES the exact [ConversionStep], the target fact query, the current final [EOOk f], the one [convert_const]
   success, and the exact current [ExprFact] — is [deep_nested_convsuccess_at] / [deep_nested_chain_success_evidence]
...
(* §3 (REPAIR 11) the EXACT per-conversion valid-chain success bundle: the FULL [retained_convsuccess_closure]
   evidence for one member — current [EConvert] view, current final [EOOk f], exact [ConversionStep], operand
   [SuffixMember], tail [EOOk opf], final [EOOk opf], tail=final query equality, ONE [convert_const] success on the
   exact target fact, and [f] the EXACT current final [ExprFact]. *)
Definition nested_success_bundle (ts : GoAST.TypeSyntax) (x : GoExpr) : Prop :=
  exists (wm : WorkMember (ep_work (build_expression_phase
                  (build_compilation_input deep_nested_program (GoIndex.index_program deep_nested_program)))))
         (rest : list (ExprWork (build_compilation_input deep_nested_program (GoIndex.index_program deep_nested_program))))
         (acc_rest : OutcomeAccumulator (ep_work (build_expression_phase
    ...
    /\ total_forest_outcome_at (ep_ot (build_expression_phase
         (build_compilation_input deep_nested_program (GoIndex.index_program deep_nested_program))))
         (proj1_sig (cs_operand_suffix step)) = oa_total acc_rest (cs_operand_suffix step)
    /\ convert_const (tnf_type (type_name_fact_at_table (ep_tnft (build_expression_phase
         (build_compilation_input deep_nested_program (GoIndex.index_program deep_nested_program))))
         (cw_target_ref (cs_conversion step)))) (ef_const_status opf) = Some tc
    /\ f = mkExprFact (CITyped (tnf_type (type_name_fact_at_table (ep_tnft (build_expression_phase
         (build_compilation_input deep_nested_program (GoIndex.index_program deep_nested_program))))
         (cw_target_ref (cs_conversion step)))) tc)
             (use_resolved_of_ci (expr_ref_role (ew_expr_ref (proj1_sig wm)))
                (CITyped (tnf_type (type_name_fact_at_table (ep_tnft (build_expression_phase
                   (build_compilation_input deep_nested_program (GoIndex.index_program deep_nested_program))))
                   (cw_target_ref (cs_conversion step)))) tc)).

(* §3.2 the EXACT concrete helper: any valid deep_nested conversion occurrence instantiates the FULL bundle
   (via [retained_convsuccess_closure] on the phase's OWN [ep_ot]).  No reduced projection. *)
...
(* §3.3 the EXACT concrete aggregate: ALL FOUR valid-chain conversions (int8/int16/int32/int64) instantiate the
   ...
   convert_const result, and current final ExprFact.  [deep_nested_all_ok] remains a short shape corollary. *)
Theorem deep_nested_chain_success_evidence :
  nested_success_bundle (GoAST.tsyn GoNames.TNint8) (EInt 5)
  /\ nested_success_bundle (GoAST.tsyn GoNames.TNint16) (EConvert (GoAST.tsyn GoNames.TNint8) (EInt 5))
  /\ nested_success_bundle (GoAST.tsyn GoNames.TNint32)
       (EConvert (GoAST.tsyn GoNames.TNint16) (EConvert (GoAST.tsyn GoNames.TNint8) (EInt 5)))
  /\ nested_success_bundle (GoAST.tsyn GoNames.TNint64)
       (EConvert (GoAST.tsyn GoNames.TNint32) (EConvert (GoAST.tsyn GoNames.TNint16) (EConvert (GoAST.tsyn GoNames.TNint8) (EInt 5)))).
Proof.
  split; [ | split; [ | split ] ].
  - destruct (GoIndex.source_occurrence_at deep_nested_src 11) as [occ|] eqn:Eo; [| vm_compute in Eo; discriminate Eo].
    apply (deep_nested_convsuccess_at 11 (GoAST.tsyn GoNames.TNint8) (EInt 5) occ Eo);
      [ vm_compute in Eo; injection Eo as <-; vm_compute; reflexivity
      | vm_compute in Eo; injection Eo as <-; eexists; vm_compute; reflexivity ].
  - destruct (GoIndex.source_occurrence_at deep_nested_src 9) as [occ|] eqn:Eo; [| vm_compute in Eo; discriminate Eo].
    apply (deep_nested_convsuccess_at 9 (GoAST.tsyn GoNames.TNint16)
             (EConvert (GoAST.tsyn GoNames.TNint8) (EInt 5)) occ Eo);
      [ vm_compute in Eo; injection Eo as <-; vm_compute; reflexivity
      | vm_compute in Eo; injection Eo as <-; eexists; vm_compute; reflexivity ].
  - destruct (GoIndex.source_occurrence_at deep_nested_src 7) as [occ|] eqn:Eo; [| vm_compute in Eo; discriminate Eo].
    apply (deep_nested_convsuccess_at 7 (GoAST.tsyn GoNames.TNint32)
             (EConvert (GoAST.tsyn GoNames.TNint16) (EConvert (GoAST.tsyn GoNames.TNint8) (EInt 5))) occ Eo);
      [ vm_compute in Eo; injection Eo as <-; vm_compute; reflexivity
      | vm_compute in Eo; injection Eo as <-; eexists; vm_compute; reflexivity ].
  - destruct (GoIndex.source_occurrence_at deep_nested_src 5) as [occ|] eqn:Eo; [| vm_compute in Eo; discriminate Eo].
    apply (deep_nested_convsuccess_at 5 (GoAST.tsyn GoNames.TNint64)
...
Theorem deep_nested_seals_eft :
  exists cp Hcp,
    go_compile deep_nested_program = CompiledOk cp Hcp
    /\ ef_expr_facts (cp_facts cp) = feft_table (ep_eft (cp_phase cp)).
Proof.
  destruct (go_compile_complete deep_nested_program deep_nested_compiles) as [cp [Hcp Hgo]].
