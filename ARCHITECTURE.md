...
`make check` verifies the **working tree** (it materializes the tracked files' working-tree content — via
`git ls-files` through tar, so it reflects uncommitted edits and excludes untracked residue — rebuilds the
