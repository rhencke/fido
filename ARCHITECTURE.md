...
```
  ...
  GoIndex        the ONE structural occurrence-identity + navigation authority, DERIVED from one immutable
                 ...
                 hot path) and delegating to the SAME expr_typedb/const_info resolver, and is proved EQUAL to
                 program_typedb (no semantic judgment duplicated, no generated byte changed).
