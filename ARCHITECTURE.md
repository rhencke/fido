...
| **GoCompile** | the SOLE meeting point of GoIndex identity + GoTypes semantics (owns the per-occurrence `occ_arg_typedb` + its `occs_*_typedb_eq` traversal bridge, moved off GoTypes); whole-program map-based typing + one-pass `PackageMap` grouping (a single `FM.fold` into `PackageSummary`, characterized EXACTLY: count = sum over the package's files, no empty package) + the fresh-build output preflight over the factored source rules (`GoCompile p := fresh_build_preflight_ok p /\ SourceProgramValid p`, via GoTypes); `go_compile` sound/complete, its accept/error class invariant under file insertion order; the retained `elaborate` produces ONE `ElaborationFacts` (indexed BY the retained `IndexedProgram`) carrying the occurrence-keyed expression facts + the occurrence-keyed type-name facts (each conversion's source name resolved via the ONE compiler-owned `predeclared_type` context to a semantic `GoType`; `byte`→`uint8`, `rune`→`int32`) + package `main`-ref buckets + the fresh-build preflight witness, exposed by canonical projection; the production expression path is ONE `ExpressionPhase` built from ONE retained `CompilationInput`: the PROOF-CARRYING `ExprOutcomeTable` (`build_outcome_table`, pairing the `build_outcomes` `fold_right` over the retained source-order visit with the ONE carried invariant — the DIRECT cause `eot_caused` (every stored outcome justified by the `OutcomeCause` relation, read off the table + the operand's already-computed outcome + one `convert_const`, NEVER `local_conv_failure`/a resolver/a recursive `const_info`) together with its EXACT domain `eot_dom` (every key is a visited EXPRESSION occurrence — the domain is EXACTLY the `ExprWork` key set, `eot_domain_iff_work`, so an extra/foreign/wrong-kind key is uninhabitable); the index-free source specification is REACHED from the carried cause by the SEPARATE exactness theorem `outcomes_caused_matches`, never carried a second time) that CONSUMES the once-built `TypeNameFactTable` object (`build_type_name_fact_table`): each conversion step queries the table at its retained target `TypeNameRef` (the TOTAL `type_name_fact_at_table` query, no fallback), reads its operand's ALREADY-COMPUTED outcome from the PROCESSED SUFFIX at the retained operand `ExprRef` (the TOTAL `from_some` of the operand-closure proof — missing-operand impossible, never a fallback), and calls `convert_const` ONCE per conversion (no raw NodeKey arithmetic, no second visit, no `const_info` on the live path, no per-ancestor re-evaluation, no `index_program` reconstruction in the phase closure); the invalid-conversion outcome (`EOConvFail`) CARRIES the exact conversion / target / operand refs, so the diagnostic PROJECTS them — the operand ref is a field of `DRInvalidConversion`, nothing re-minted; the TOTAL fact projection (`phase_expr_facts`, via `total_outcome_at` over the exact typed `ExprWork` domain) and the TOTAL diagnostic projection (`phase_expr_diags`, via `awork_diags` over the typed-work annotation — keyed by each work item's OWN carried `ew_expr_ref`, so there is NO `as_expr` call and NO fail-open `None` branch) both read that SAME `ExprOutcomeTable` inside the one phase (`facts_and_diags_share_phase`, object identity — no fail-open `find`), the phase's `TypeNameFactTable` (`ep_tnft`) is sealed into `ElaborationFacts` by OBJECT IDENTITY (`elaborate_ok_seals_tnfacts`) and its `ExprFactTable` likewise (`elaborate_ok_seals_facts`, the retained OBJECT `ef_expr_facts facts = ep_eft phase`, not a map equality — no rebuild after the decision), a stored `EOConvFail`'s cause is read DIRECTLY off the phase as a PROJECTION of the carried `OutcomeCause` (`phase_convfail_cause`, plus the success and blocked-child cause theorems `phase_convok_cause`/`phase_childfail_cause`), and the erased report retains the target source spelling; `CompilableProgram` RETAINS index + facts (`cp_prov`); NO `cf_pkg_name` (package clause is source-owned) | be a boolean; accept per-file partially; re-scan the file list per package (O(files²)); hide package grouping / entry status in a raw node; store a typed copy of the program; reconstruct the index rather than retain it |
