...
that live path for only a small expression class (a binop tree over locals, int/int64/uint64 literals, and
int64/uint64 complement `^x`) — every other shape is printed by the trusted OCaml `pp_expr`. And even for that class
