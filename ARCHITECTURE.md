...
```
  ...
  Emit.Image the COMPLETE module: exact root go.mod bytes (Emit.module_bytes) + a finite map from FilePath.T to
                 ...
                 empty; there is NO nonemptiness claim.  the image REPRESENTATION and its constructor are PRIVATE (Charter §22/§24) — the only mint is `Emit.of_safe` from an actual `Safe.Program`, and the image RETAINS that exact certificate, so provenance is a projection of it;
