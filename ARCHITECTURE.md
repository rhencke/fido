...
```
  ...
  Emit.Image the COMPLETE module: exact root go.mod bytes (Emit.module_bytes) + a finite map from FilePath.T to
                 ...
                 empty; there is NO nonemptiness claim.  the raw `Emit.Mint.Token` constructor is PRIVATE and `Emit.Mint.issue` is the sole authority-producing operation (A006 / D-26); `Emit.Image` is a REDUCIBLE carrier retaining the exact `Safe.Program`, exact bytes and that exact token, so provenance is a projection and its pack constructor — which is not a mint — cannot be applied to foreign bytes;
...
| **Emit.Image** | the complete module (exact go.mod bytes + a possibly-empty .go map), authority-gated (A006 / D-26: the raw `Emit.Mint.Token` constructor is PRIVATE and `Emit.Mint.issue` is the SOLE authority-producing operation; the image retains the exact `Safe.Program`, exact bytes and that exact token, so `Emit.provenance` is a PROJECTION of what it retains. The pack constructor is NOT private — it is a reducible carrier constructor, and what stops it authorizing foreign bytes is that the token's indices force the payload); `Fido Materialize` typechecks its argument's `Emit.transport` AND rejects any argument with an axiomatic assumption closure | be an arbitrary-map escape that bypasses Safe.Program; invent go.mod in the sink; make a nonemptiness claim; accept a raw transport, or a same-typed image built from a forged (axiomatic) proof, at the transport boundary |
...
resolved `Typing.SemanticType`). C5 is the permanent `Machine` base (FCB Roadmap), NOT a numeric-scope phase;
`uintptr` stays OUT and admitting it requires REOPENING the adopted ADR-0001 or a replacement target decision
plus its full inclusion price. Exact rune constants/literals are likewise a separate priced scope change.**
