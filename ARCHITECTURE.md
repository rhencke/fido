...
metric-index, program and population tables are GENERATED_VIEW (regenerated and byte-compared by the
ordinary and staged gates, so a hand edit or stale regeneration cannot publish), the scenario and
opportunity files are STATUS_LEDGER, and `performance-bases.tsv` is the BASIS_REGISTRY — each basis digest
bound to its exact source commit, verified against Git, with exactly one CURRENT basis joined to the
candidate digest. Each registered topology version owns one exact measured graph — exact event identities,
parents, complete predecessor sets, boundary starts and terminal — so a required event cannot be omitted,
renamed, re-parented, or given different edges. Every boundary has its own wall (terminal end minus
boundary start, never the reused root wall); the critical path is the longest predecessor path ending at
the exact boundary terminal; temporal overlap (work minus interval union) and work outside the critical
