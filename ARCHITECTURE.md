...
```
  ...
  GoIndex        the ONE structural occurrence-identity + navigation authority, DERIVED from one immutable
                 ...
                 index_program, threads it through visit_file, delegates each decision to the SAME
                 expr_typedb/const_info resolver via the total query API, and is proved EQUAL to program_typedb
                 (no semantic judgment duplicated, no generated byte changed).
