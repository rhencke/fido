...
representable/unrepresentable path fixtures; the Collections standard-map foundation (FilePath ordered-key law + extensionally-equal maps enumerate identically) and the GoAST duplicate-rejecting `filemap_of_nodes` (sound + complete + exact + permutation-stable) + `build_program`; GoTypes — the one type authority: zero-sign constant equality, `default_const`
