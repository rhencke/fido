...
| **GoCompile** | the SOLE meeting point of GoIndex identity + GoTypes semantics (owns the per-occurrence `occ_arg_typedb` + its `occs_*_typedb_eq` traversal bridge, moved off GoTypes); whole-program map-based typing + one-pass `PackageMap` grouping (a single `FM.fold` into `PackageSummary`, characterized EXACTLY: count = sum over the package's files, no empty package) + the fresh-build output preflight over the factored source rules (`GoCompile p := fresh_build_preflight_ok p /\ SourceProgramValid p`, via GoTypes); `go_compile` sound/complete, its accept/error class invariant under file insertion order; the retained `elaborate` produces ONE `ElaborationFacts` (indexed BY the retained `IndexedProgram`) carrying the occurrence-keyed expression facts + the occurrence-keyed type-name facts (each conversion's source name resolved via the ONE compiler-owned `predeclared_type` context to a semantic `GoType`; `byte`→`uint8`, `rune`→`int32`) + package `main`-ref buckets + the fresh-build preflight witness, exposed by canonical projection; the production expression path is ONE `ExpressionPhase` built from ONE retained `CompilationInput`, driven by ONE retained work forest — `prog_forest` (the `ExprWork` items of the retained visit, each carrying its own `ExprRef` plus a total `ConvRefinement` of its conversion target/operand refs; built ONCE, `map (ew_node_ref,ew_occurrence) prog_forest = filter occ_is_expr (ci_visit)` pinning order + forward domain `prog_forest_complete` + reverse domain `prog_forest_sound` + key-NoDup `prog_forest_nodup`), which the outcome fold, the facts, the annotation, and the diagnostics ALL consume (no second work discovery, no reminted conversion ref): the PROOF-CARRYING `ForestOutcomeTable` (`build_forest_outcome_table`, folding `prog_forest` with the ONE carried invariant — the DIRECT cause `fot_caused` (every stored outcome justified by the `OutcomeCause` relation, read off the table + the operand's already-computed outcome + one `convert_const`, NEVER `local_conv_failure`/a resolver/a recursive `const_info`) together with its EXACT domain `fot_dom` (every key is a visited EXPRESSION occurrence — the domain is EXACTLY membership in the retained enumeration `prog_forest`, `fot_domain_iff_forest`, so an extra/foreign key is uninhabitable and a non-expression key is absent `fot_nonexpr_absent`); the index-free source specification is REACHED from the carried cause by the SEPARATE exactness theorem `outcomes_caused_matches`, never carried a second time) that CONSUMES the once-built `TypeNameFactTable` object (`build_type_name_fact_table`): each conversion step queries the table at its retained target `TypeNameRef` (the TOTAL `type_name_fact_at_table` query, no fallback), reads its operand's ALREADY-COMPUTED outcome from the PROCESSED SUFFIX at the retained operand `ExprRef` (the retained forest item is in the processed tail, `prog_forest_operand_in_tail`; the TOTAL `from_some` of the operand-closure proof — missing-operand impossible, never a fallback), and calls `convert_const` ONCE per conversion (no raw NodeKey arithmetic, no second visit, no `const_info` on the live path, no per-ancestor re-evaluation, no `index_program` reconstruction in the phase closure); the invalid-conversion outcome (`EOConvFail`) CARRIES the exact conversion / target / operand refs, so the diagnostic PROJECTS them — the operand ref is a field of `DRInvalidConversion`, nothing re-minted; the TOTAL fact projection (`forest_facts`, folding `prog_forest` via `total_forest_outcome_at`) and the TOTAL diagnostic projection (`forest_diags`, via `forest_awork_diags` over the retained annotated forest `prog_forest_awork` — keyed by each work item's OWN carried `ew_expr_ref`, so there is NO `as_expr` call and NO fail-open `None` branch) both read that SAME `ForestOutcomeTable` inside the one phase, which RETAINS the whole flow as objects with dependent provenance (`ep_work`/`ep_ot`/`ep_eft`/`ep_awork`/`ep_diags` each tied to the canonical build, so a foreign component is UNREPRESENTABLE; `ep_facts = eft_map ep_eft`; `facts_and_diags_share_phase`, object identity — no fail-open `find`), the phase's `TypeNameFactTable` (`ep_tnft`) is sealed into `ElaborationFacts` by OBJECT IDENTITY (`elaborate_ok_seals_tnfacts`) and its `ExprFactTable` likewise (`elaborate_ok_seals_facts`, the retained OBJECT `ef_expr_facts facts = ep_eft phase`, not a map equality — no rebuild after the decision), a stored `EOConvFail`'s cause is read DIRECTLY off the phase as a PROJECTION of the carried `OutcomeCause` keyed by the WORK item (`phase_convfail_cause`, plus the success and blocked-child cause theorems `phase_convok_cause`/`phase_childfail_cause`), and the erased report retains the target source spelling; `CompilableProgram` RETAINS index + facts (`cp_prov`); NO `cf_pkg_name` (package clause is source-owned) | be a boolean; accept per-file partially; re-scan the file list per package (O(files²)); hide package grouping / entry status in a raw node; store a typed copy of the program; reconstruct the index rather than retain it |
