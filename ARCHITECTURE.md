...
| **GoAST** | `ModuleSpec` + a possibly-EMPTY program map + raw `GoDecl` (a `func main` form) over `EBool`/`EInt`/`ENeg`/`EString`/`EFloat` (a bare float literal carrying an INTRINSIC bounded-canonical finite decimal `coeff·10^exp`, `\|coeff\|<10^40`, `\|exp\|≤4096`)/`EComplex` (a semantic complex literal, a PAIR of `DecimalFloat` components spelled `complex(re, im)`)/ONE `EConvert TypeSyntax GoExpr` (a source-shaped explicit conversion naming a SOURCE type — the closed sixteen-name lexical class carried as a `TypeSyntax`: the fourteen numeric names plus the `byte`/`rune` SOURCE ALIASES — over any `GoExpr` operand; the SOURCE name never decides its semantic type); a SOURCE-owned package clause (`source_package`, `PkgMain`) + an intrinsically-empty import section (`source_imports`); key-uniqueness intrinsic (the map key) | carry an entry-point flag / a compiler-derived package name / package GROUPING / a raw path stored in the mapped value; an ARBITRARY (non-`main`) package clause or non-empty imports; a nonemptiness restriction; a second tree; a type on a raw literal; a resolved SEMANTIC target (`GoType`, or a `byte`/`rune` collapsed to `uint8`/`int32`) baked into raw conversion syntax; a source-name→`GoType` table in the AST; arithmetic / imaginary-literal / `real`/`imag` / NaN / Inf syntax |
...
| **GoCompile** | the SOLE meeting point of GoIndex identity + GoTypes semantics (owns the per-occurrence `occ_arg_typedb` + its `occs_*_typedb_eq` traversal bridge, moved off GoTypes); whole-program map-based typing + one-pass `PackageMap` grouping (a single `FM.fold` into `PackageSummary`, characterized EXACTLY: count = sum over the package's files, no empty package) + the fresh-build output preflight over the factored source rules (`GoCompile p := fresh_build_preflight_ok p /\ SourceProgramValid p`, via GoTypes); `go_compile` sound/complete, its accept/error class invariant under file insertion order; the retained `elaborate` produces ONE `ElaborationFacts` (indexed BY the retained `IndexedProgram`) carrying the occurrence-keyed expression facts + the occurrence-keyed type-name facts (each conversion's source name resolved via the ONE compiler-owned `predeclared_type` context to a semantic `GoType`; `byte`→`uint8`, `rune`→`int32`) + package `main`-ref buckets + the fresh-build preflight witness, exposed by canonical projection; the production expression path is ONE `ExpressionPhase` built from ONE retained `CompilationInput`, driven by ONE proof-carrying retained work forest OBJECT — the `ExprWorkForest` record (`build_expr_work_forest`; its stored `ewf_blocks`/`ewf_items` with `ewf_items = concat ewf_blocks`, forward domain `ewf_forward` + reverse domain `ewf_reverse` + key-NoDup `ewf_keys_nodup` + per-file/flat order + operand-in-suffix `ewf_operand_in_tail`, all carried as FIELDS; `build_forest_blocks` stays PRIVATE inside, returning its proof INTO the record — no `proj1_sig` discard) whose IDENTITY role is a SEPARATE carried field, the exact standard work-member index `ewf_index : ExprWorkIndex ewf_items` — the ONE identity authority over the work items: a pinned-stdlib `GoIndex.NodeKeyMapBase` (`FMapAVL`) map built ONCE by `build_work_index` from the ALREADY-BUILT item list (so it can neither re-traverse `ci_visit` nor re-run `build_forest_blocks`), TOTAL with no option/fallback/empty-default precisely because it DEMANDS `ewf_keys_nodup` as a PROOF ARGUMENT, and OVERWRITE-FREE (`work_index_fresh`/`work_index_add_fresh` prove every `add` writes an absent key); the record is INDEXED BY `ewf_items` and `ewi_exact` pins map to list in BOTH directions, so a foreign map is not pairable with a forest and a duplicate-keyed list has no index at all; `ewi_domain` and `ewi_key_inj` are DERIVED (never stored — no second domain or uniqueness authority, and `ewf_key_inj` is now that map-derived theorem). The ordered `ewf_items` therefore keeps its ORDER role ONLY: the TOTAL member queries `index_member_at`/`forest_index_member_at` are ONE `NodeKeyMapBase.find` with the impossible `None` discharged by a Prop existence hypothesis, `build_conversion_work` queries the index at the key of the operand `ExprRef` the item ALREADY CARRIES (`ew_conv`) rather than a separately computed value, and NO `List.find`/`existsb`/recursive key scan exists anywhere in the work-member lookup path (`index_no_foreign`/`index_nonexpr_absent` exclude foreign and wrong-kind keys), each item an `ExprWork` carrying its own `ExprRef` plus a total `ConversionWork` view of its conversion target/operand refs (with the exact operand `WorkMember` + the target-before-operand SOURCE order `cw_target_before_op`, NOT a suffix-membership proof — processed-suffix membership is the SEPARATE `ewf_operand_in_tail`), which the outcome fold, the facts, the annotation, and the diagnostics ALL consume by RECEIVING the exact object as a parameter (no second work discovery, no reminted conversion ref): the PROOF-CARRYING `ForestOutcomeTable forest tnft` (`build_forest_outcome_table`, folding the object's `ewf_items` into the proof-carrying `OutcomeAccumulator` `fot_acc` PAIRED WITH the `OutcomeTrace` `fot_trace` that BUILT it (INDEXED by `fot_acc`, so the accumulator and its causal predecessor chain are NOT freely pairable — a foreign rest accumulator that merely reproduces a head outcome cannot be attached); the per-member RETAINED direct cause is a PROJECTION of the trace (`total_forest_outcome_cause` returning each member's `RetainedMemberCause` — its exact suffix split `ewf_items = prefix ++ current :: rest`, the AUTHENTICATED prior `OutcomeAccumulator` for `rest`, the member/suffix-indexed `StepCause` reading the operand outcome THROUGH the exact operand `SuffixMember` via the total `oa_total acc_rest`, AND the tail-to-final QUERY PRESERVATION — NEVER `local_conv_failure`/a resolver/a recursive `const_info`) together with its EXACT domain `fot_dom` (`oa_domain`: every key is a visited EXPRESSION occurrence — the domain is EXACTLY membership in `ewf_items`, `fot_domain_iff_forest`, so an extra/foreign key is uninhabitable and a non-expression key is absent `fot_nonexpr_absent`); the index-free source specification is REACHED per member by the SEPARATE match `total_forest_outcome_at_matches` (`trace_match`, projected from the trace), never carried a second time) that CONSUMES the once-built `TypeNameFactTable` object (`build_type_name_fact_table`): each conversion step queries the table at its retained target `TypeNameRef` (the TOTAL `type_name_fact_at_table` query, no fallback), reads its operand's ALREADY-COMPUTED outcome THROUGH the exact operand `SuffixMember` of the processed tail (`cs_operand_suffix`, whose tail-membership is `ewf_operand_in_tail`; the TOTAL `oa_total acc_rest` query on the prior accumulator — missing-operand impossible by `oa_covers`, never a raw `find`/`operand_key`/fallback), and calls `convert_const` ONCE per conversion (no raw NodeKey arithmetic, no second visit, no `const_info` on the live path, no per-ancestor re-evaluation, no `index_program` reconstruction in the phase closure); the invalid-conversion outcome (`EOConvFail`) CARRIES the exact conversion / target / operand refs, so the diagnostic PROJECTS them — the operand ref is a field of `DRInvalidConversion`, nothing re-minted; the TOTAL fact projection (`forest_facts`, folding `ewf_items` via `total_forest_outcome_at`) and the TOTAL diagnostic projection (the retained `ExpressionDiagnostics` object's stored list, via `forest_awork_diags` over the retained `AnnotatedExprWorkForest` OBJECT `build_annotated_work_forest` — a proof-carrying record whose members ARE `ewf_items` in order (`aewf_members`), whose `aewf_diag_fold` ties them to the one-pass `annotate_program` (so diagnostics CONSUME the object — no `proj1_sig` discard, no re-annotation), and which carries context soundness/same-file/nearest-first/nodup; keyed by each work item's OWN carried `ew_expr_ref`, so NO `as_expr` call and NO fail-open `None` branch) both read that SAME `ForestOutcomeTable` inside the one phase, which RETAINS the whole flow as a DEPENDENT CHAIN of objects — `ep_ot : ForestOutcomeTable ep_work ep_tnft`, `ep_awork : AnnotatedExprWorkForest ep_work`, `ep_eft : ForestExprFactTable ep_work ep_ot`, `ep_diag : ExpressionDiagnostics ep_awork ep_ot` — each TYPED by the exact prior object it consumes (NO provenance-equality field; the causal chain is the dependent types, shown definitionally by `phase_ot_consumes_work`/`phase_awork_consumes_work`/`phase_eft_consumes_work_ot`/`phase_diag_consumes_awork_ot`), so a foreign component is UNREPRESENTABLE by type mismatch (`ep_facts = eft_map (feft_table ep_eft)`; `facts_and_diags_share_phase`, object identity — no fail-open `find`); the phase's `TypeNameFactTable` (`ep_tnft`) is sealed into `ElaborationFacts` by OBJECT IDENTITY (`elaborate_ok_seals_tnfacts`) and its `ExprFactTable` likewise (`elaborate_ok_seals_facts`, the retained OBJECT `ef_expr_facts facts = feft_table (ep_eft phase)`, not a map equality — no rebuild after the decision), a stored outcome's cause is PROJECTED from the retained `OutcomeTrace` keyed by the WORK member (`total_forest_outcome_cause` returning each member's exact `RetainedMemberCause` — its suffix split + the AUTHENTICATED tail `OutcomeAccumulator` + the `StepCause` producing the FINAL outcome + the tail-to-final query preservation, so a foreign tail accumulator cannot satisfy it; `final_operand_outcome` closes a conversion's operand into the final table); the direct cause is projected axiom-free by `StepCause_convfail_inv`/`StepCause_childfail_inv`/`StepCause_ok_conv_inv`, each reading the operand outcome THROUGH the exact operand `SuffixMember` via `oa_total acc_rest`), and the erased report retains the target source spelling; `CompilableProgram` RETAINS index + facts (`cp_prov`); NO `cf_pkg_name` (package clause is source-owned) | be a boolean; accept per-file partially; re-scan the file list per package (O(files²)); hide package grouping / entry status in a raw node; store a typed copy of the program; reconstruct the index rather than retain it; recover a keyed work member by SCANNING the ordered item list (a `list + NoDup` standing in for an identity map) |
...
| **GoRender** | render decls + the SOURCE-owned package clause (from the file's OWN `source_package` via `render_package_clause`, `PkgMain` → `main` — NOT a derived/deduced name); render go.mod from the ModuleSpec; a conversion as `<source-name>(<inner>)` rendering the exact SOURCE spelling from the `TypeSyntax` (`render_type_syntax` = `render_stn`; `byte`/`rune` render their OWN spelling, NEVER the resolved `uint8`/`int32`); ONE canonical float decimal spelling (`0.0`; else `<signed-coeff>.0e<±exp>`) with an INDEPENDENT decoder proving `decode(render d) = Some d`, and ONE canonical complex spelling `complex(<real>, <imag>)` (both components via that decimal spelling) with an INDEPENDENT complex decoder + semantic round trip; header exact first line (go.mod and .go); `render_const_info_denotes` / `render_resolved_expr_denotes` (spelling ↔ ConstInfo ↔ value/resolved type; the conversion case via `convert_const` over the rendered SOURCE spelling — one determinism theorem `render_const_info_denotes_functional` over the sixteen source names; all via the ONE `RenderedConstInfoDenotes`) | tokenize/lex/parse/round-trip; deduce packages/entry; invoke a formatter; add require/replace/toolchain to go.mod |
