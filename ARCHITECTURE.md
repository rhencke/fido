...
| **Emit.Image** | the complete module (exact go.mod bytes + a possibly-empty .go map), provenance-gated (`Emit.provenance` proves BOTH came from `Emit.of_safe`; `the raw `Emit.Mint.Token` constructor is PRIVATE and `Emit.Mint.issue` the sole authority-producing operation; the image retains the exact `Safe.Program`, exact bytes and that exact token, and its reducible pack constructor is not a mint); `Fido Materialize` typechecks its argument's `Emit.transport` AND rejects any argument with an axiomatic assumption closure | be an arbitrary-map escape that bypasses Safe.Program; invent go.mod in the sink; make a nonemptiness claim; accept a raw transport, or a same-typed image built from a forged (axiomatic) proof, at the transport boundary |
