...
```
  ...
  GoIndex        the ONE structural occurrence-identity + navigation authority, DERIVED from one immutable
                 ...
                 The GoTypes typing layer consumes it MINIMALLY: `indexed_program_typedb` let-binds ONE
                 index_program p and folds visit_file's (NodeRef, syntax) pairs, taking each occurrence's ROLE
                 from the index THROUGH the reference (node_role idx — an outer-FileMap + inner-PositiveMap
                 lookup in the PRECOMPUTED si_outer, no rebuild) and its SYNTAX from the DELIVERED fragment
                 (view_expr — no node_at/source_occurrence_of_ref recovery), delegating to the SAME
                 expr_typedb/const_info resolver, and is proved EQUAL to program_typedb (no semantic judgment
                 duplicated, no generated byte changed).
  ...
  GoTypes        the ONE Go type-system authority — EVIDENCE over the raw GoAST, never a typed AST.  The
                 ...
                 F64 — the double-round scar); a complex conversion (EComplexConvert ct e) rounds EACH component
                 ONCE at ct's component format.  resolve_const_info : ConstInfo -> option ResolvedConst is the USE
  ...
  GoRender       the direct renderer.  It renders each GoSourceFile to bytes (the package clause from the
                 ...
                 a complex conversion as `complex64(<inner>)`/`complex128(<inner>)`;
...
| **GoCompile** | whole-program map-based typing + one-pass `PackageMap` grouping (a single `FM.fold` into `PackageSummary`, characterized EXACTLY: count = sum over the package's files, no empty package) + exactly-one-main (`GoCompile p := ProgValid p`, via GoTypes); `go_compile` sound/complete, its accept/error class invariant under file insertion order; compiled evidence exposes typing by canonical projection (no `CompilationFacts`/`cf_pkg_name` — package clause is source-owned) | be a boolean; accept per-file partially; re-scan the file list per package (O(files²)); hide package grouping / entry status in a raw node; store a typed copy of the program |
