...
and numeric→float64 `is_num_to_f64_ref` (over int/int64/float32/uint64); NOT every producer of those surface
bytes — e.g. int/int64/uint64→float32 `is_int_to_f32_ref` stays on `pp_expr`) — every other shape
