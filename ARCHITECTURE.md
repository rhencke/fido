...
`make regenerate` rewrites it into the repo through the SAME `Fido_sink`. Both `make check` (its
`verify-generated` step) and the pre-commit hook export the Git INDEX and verify the STAGED tree: they
rebuild the pristine `generated-module`/`generated-artifact` from the staged inputs and compare the staged
`go.mod` + recursive `.go` byte-exact against it (path set + bytes, both directions) — never reading the
unstaged working tree, never auto-staging. This byte-compare is essential because `.dockerignore` hides the
committed `go.mod`/`.go` from the Buildx context, so the proof/e2e cannot incidentally validate their bytes.
The pre-commit is a PROTOTYPE boundary, bypassable with `git commit --no-verify`; a future PR CI runs the
same comparison server-side.
